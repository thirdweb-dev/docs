---
slug: /sdk/interacting-with-contracts/token
title: Token
---

import CodeSnippetWithSummary from "../../../../../src/components/CodeSnippetWithSummary";

# Token

Learn how to interact with your [Token](/pre-built-contracts/token) contract in the SDK.

## Create a Token Contract

<CodeSnippetWithSummary contract={"ContractDeployer"} name={"deployToken"} />

## Getting the contract in your application

To start using your Token contract inside your application, you need to use its contract address.
You can get the contract address from the [dashboard](https://thirdweb.com/dashboard).

<CodeSnippetWithSummary
  contract={"Token"}
  name={"initializing the SDK"}
  isGetContractCode
  dontShowSummary
  snippetOverrides={{
    unity: `Contract contract = sdk.GetContract("{{contract_address}}");`,
  }}
/>

## Minting Tokens

### Mint tokens to a specified wallet

<CodeSnippetWithSummary contract={"Token"} name={"mintTo"} dontShowSummary />

### Mint tokens to many wallets in one transaction (batch)

<CodeSnippetWithSummary contract={"Token"} name={"mintBatchTo"} />

## Token Metadata

Get the metadata about the token itself, such as the name, symbol, and decimals.

<CodeSnippetWithSummary
  contract={"Token"}
  name={"get"}
  dontShowSummary
  showHr={false}
/>

You can get the total supply of the token too.

<CodeSnippetWithSummary
  contract={"TokenDrop"}
  name={"totalSupply"}
  dontShowSummary
/>

## Token Balance

### Balance of the connected wallet

<CodeSnippetWithSummary contract={"Token"} name={"balance"} />

### Balance of a specified wallet

<CodeSnippetWithSummary contract={"Token"} name={"balanceOf"} />

## Token Allowance

Allowance refers to how many tokens another address is allowed to spend from your wallet.

For example, our [Marketplace](/pre-built-contracts/marketplace) contract asks you permission to **increase your allowance** when you make a bid on an auction listing.

### Get allowance for the connected wallet

Get the number of tokens that another wallet can spend on behalf of the connected wallet.

<CodeSnippetWithSummary contract={"Token"} name={"allowance"} dontShowSummary />

### Get allowance for a specified wallet

Get the number of tokens that another wallet can spend on behalf of the specified wallet.

<CodeSnippetWithSummary
  contract={"Token"}
  name={"allowanceOf"}
  dontShowSummary
/>

### Set Allowance

Specify how many tokens another wallet is allowed to spend on behalf of the connected wallet.

<CodeSnippetWithSummary
  contract={"Token"}
  name={"setAllowance"}
  dontShowSummary
/>

## Transfer Tokens

You can transfer tokens from one wallet to another or send tokens to a smart contract address.

### Transfer from the connected wallet

<CodeSnippetWithSummary contract={"Token"} name={"transfer"} dontShowSummary />

### Transfer from the connected wallet in batch

<CodeSnippetWithSummary contract={"Token"} name={"transferBatch"} />

### Transfer from a specified wallet

<CodeSnippetWithSummary contract={"Token"} name={"transferFrom"} />

## Burning Tokens

Burning tokens takes a specified amount of tokens out of the circulating supply.

### Burn from the connected wallet

<CodeSnippetWithSummary contract={"Token"} name={"burn"} />

### Burn from a specified wallet

<CodeSnippetWithSummary contract={"Token"} name={"burnFrom"} showHr={false} />
