---
slug: /sdk/interacting-with-contracts/erc1155/erc1155
title: Standard
---

import FeatureCodeSnippets from "../../../../../src/components/FeatureCodeSnippets";
import CodeSnippetWithSummary from "../../../../../src/components/CodeSnippetWithSummary";
import QuickstartCard from "../../../../../src/components/QuickstartCard";

# ERC1155 Standard

You can utilize these features of the SDK if your contract implements the [ERC1155](/extensions/erc-1155/standard) standard.

<div className="row" style={{ marginBottom: 24 }}>
  <div className="col col--12" style={{ marginTop: 8 }}>
    <QuickstartCard
      name="Learn how to implement ERC1155 in your smart contract"
      link={"/extensions/erc-1155/standard"}
      image="/assets/icons/general.png"
    />
  </div>
</div>

## View NFT Balance

Get a wallet's NFT balance (number of NFTs in this contract owned by the wallet) for a specific token ID.

<CodeSnippetWithSummary
  contract={`ERC1155`}
  name={`balanceOf`}
  isFeatureSnippet={true}
  isGetContractCode={false}
  showHr={true}
  dontShowSummary
/>

## Get NFT Metadata

Read the URI this NFT points to and fetch that data automatically.

If the metadata is stored on IPFS, this function uses our IPFS gateway (or the one you specify) to read the data.

<CodeSnippetWithSummary
  contract={`ERC1155`}
  name={`get`}
  isFeatureSnippet={true}
  isGetContractCode={false}
  showHr={true}
  dontShowSummary
/>

## Transfer NFTs

Transfer an NFT from the connected wallet to another wallet.

<CodeSnippetWithSummary
  contract={`ERC1155`}
  name={`transfer`}
  isFeatureSnippet={true}
  isGetContractCode={false}
  showHr={false}
  dontShowSummary
/>

## Airdrop NFTs

Transfer NFTs to a list of wallets in one transaction.

<CodeSnippetWithSummary
  contract={`ERC1155`}
  name={`airdrop`}
  isFeatureSnippet={true}
  isGetContractCode={false}
  showHr={false}
  dontShowSummary
/>
