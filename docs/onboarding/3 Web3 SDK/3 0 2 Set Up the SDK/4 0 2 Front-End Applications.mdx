...
```jsx
import { ChainId, ThirdwebSDKProvider, useProvider } from "@thirdweb-dev/react";

// This is the chainId your dApp will work on.
const activeChainId = ChainId.Mainnet;

function MyApp({ Component, pageProps }) {
  // If you want to use any custom provider, you can add it here.
  const provider = useProvider();

  return (
    <ThirdwebSDKProvider desiredChainId={activeChainId} provider={provider}>
      {/* Your App Goes Here */}
    </ThirdwebSDKProvider>
  );
}
