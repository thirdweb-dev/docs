...
export const MyApp = () => {
  return <ThirdwebProvider>{/* Your App Goes Here */}</ThirdwebProvider>;
};
```
...
For non-default chains, import the chain from the `@thirdweb-dev/chains` package and pass it to the `activeChain` prop.
...
You can then import your contracts into the [dashboard](/dashboard) to interact with them in your app.
