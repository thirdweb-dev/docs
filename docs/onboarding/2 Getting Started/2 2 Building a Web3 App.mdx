...
This file has our `ThirdwebProvider` configured, which allows us to use wallet connection features and interact with the desired blockchain.
...
This file contains a basic setup for users to connect their wallets to our page. It uses the `useMetamask()` to let a user connect their wallet to your app. For other ways to connect wallets, check out the [documentation](/react/category/wallet-connection).
