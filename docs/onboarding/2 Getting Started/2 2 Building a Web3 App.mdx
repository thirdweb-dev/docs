...
First, we can look at the `pages/_app.js` file.

This file has our `ThirdwebProvider` configured, which allows us to use simple wallet connection features and interact with the blockchain.
...
Additionally, you can take a look at the `pages/index.js` file.

This file contains a basic setup for users to connect their wallets to our page.
...
In the below example, we'll setup a simple claiming page so that users can connect to your website and claim NFTs from your drop. You may want to place this code
in the same component as the MetaMask connection so that users can connect their wallets and then claim.
