...
To call a payable method and send the native currency with your transaction (e.g. ETH on Ethereum), set `tx_overrides.value` in the request body to `POST /contract/<chain>/<contract_address>/write`.
...
To override the estimated gas settings for your transaction, set `tx_overrides.gasLimit` and `tx_overrides.gasPrice` in the request body to `POST /contract/<chain>/<contract_address>/write`.
