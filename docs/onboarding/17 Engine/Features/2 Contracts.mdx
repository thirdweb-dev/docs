...
To call a payable method and send the native currency with your transaction (e.g. ETH on Ethereum), set `txOverrides.value` in the request body to `POST /contract/<chain>/<contract_address>/write`.
