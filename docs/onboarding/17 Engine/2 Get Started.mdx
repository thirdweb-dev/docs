...
Use `docker` to run a Postgres Instance which Engine needs to run. Follow the below steps:

1. Create a `.env` file and add the below Env vars to it.
...
> NOTE: Make sure to change the host details on postgres connection url to `host.docker.internal` from `localhost`
