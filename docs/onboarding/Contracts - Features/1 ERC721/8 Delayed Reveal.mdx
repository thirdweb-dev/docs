---
slug: /extensions/erc-721/delayed-reveal
title: Delayed Reveal
---

import ViewContractCodeButton from "../../../src/components/ViewContractCodeButton";
import QuickstartCard from "../../../../src/components/QuickstartCard";
import ImportSnippet from "../../../src/components/ImportSnippet";

# ERC721Revealable

Encrypt the metadata of your NFTs until you reveal it by implementing `ERC721A`, `ILazyMint`, and `DelayedReveal`.

## Base Contracts Implementing This Feature

<div className="row" style={{ marginBottom: 24 }}>
  <div className="col col--6" style={{ padding: 8 }}>
    <QuickstartCard
      name="ERC721 Delayed Reveal"
      link="/extensions/base-contracts/erc-721/erc721delayedreveal"
      image="/assets/icons/general.png"
    />
  </div>
  <div className="col col--6" style={{ padding: 8 }}>
    <QuickstartCard
      name="ERC721 Drop"
      link="/extensions/base-contracts/erc-721/erc721drop"
      image="/assets/icons/general.png"
    />
  </div>
</div>

## Unlocked Features

By implementing `ERC721Claimable`, you unlock the following features in the SDK and dashboard:

| SDK Usage                                                                                | Description                                                                   |
| ---------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| [Delayed Reveals](/sdk/interacting-with-contracts/erc721/erc721droppable#delayed-reveal) | Pass in an array of NFT metadata to encrypt it and reveal it at a later time. |

## Implementing It Yourself

This section is meant for advanced users who want to write the functionality from scratch.

```solidity
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

import "@thirdweb-dev/contracts/eip/ERC721A.sol";
import "@thirdweb-dev/contracts/extension/interface/ILazyMint.sol";
import "@thirdweb-dev/contracts/extension/DelayedReveal.sol";

contract Contract is ERC721A, ILazyMint, DelayedReveal {
    constructor(
        string memory _name,
        string memory _symbol
    )
        ERC721A(
            _name,
            _symbol
        )
    {}

    function lazyMint(
        uint256 amount,
        string calldata baseURIForTokens,
        bytes calldata extraData
    ) external override returns (uint256 batchId) {
        // Your custom implementation here
    }

    function mintDelayedRevealNFT(
        address to,
        string memory beforeRevealURI,
        bytes memory encryptedPostRevealURI
    ) public {
        // Your custom implementation here
    }

    function reveal(uint256 tokenId, bytes calldata key)
        external
        override
        returns (string memory revealedURI)
    {
        // Your custom implementation here
    }

    function tokenURI(uint256 tokenId)
        public
        view
        virtual
        override
        returns (string memory)
    {
        // Your custom implementation here
    }
}
```
