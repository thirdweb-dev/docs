...
If none of the above methods are suitable for your project, you can also create your own custom wallet class by implementing the `GenericWalletSigner` interface from the `@thirdweb-dev/wallets` package for a completely custom setup.

Auth only requires the following simple wallet interface to facilitate signing and verification of JWTs:

```typescript
interface GenericWalletSigner {
...
For example, here is a minimal implementation of the basic `SignerWallet` class from the `@thirdweb-dev/auth/evm` entrypoing, which follows the `GenericSignerWallet` interface:

```typescript
import type { Ecosystem, GenericSignerWallet } from "@thirdweb-dev/wallets";

class SignerWallet implements GenericSignerWallet {
  type: Ecosystem = "evm";
