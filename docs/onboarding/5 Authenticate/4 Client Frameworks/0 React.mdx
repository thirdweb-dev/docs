...
As mentioned in the [how auth works section](/auth/how-auth-works/cookies), Auth uses cookies to securely store JWTs in the browser. This means that if our Auth backend and frontend aren't on the same domain, we may run into [CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS) issues.
