...
This guide will show you can use Auth as a custom authentication provider
for Firebase, and automatically create a document in the `users` Firestore collection when a user signs up successfully.

</p>

We'll use Next.js to build a web application that restricts access to what
content users can read and write to in a Firestore database.

</div>

<div className="heroGridImageContainer">
  <YoutubeEmbed link={"https://www.youtube.com/embed/9p7Y466c1s0"} />
</div>
</div>
...
```bash
npx thirdweb@latest create app --next --ts --evm
```
