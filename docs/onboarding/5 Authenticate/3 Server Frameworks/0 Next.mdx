...
This page will focus mainly on integrating Auth on the server-side with [Next.js](https://nextjs.org) by implementing an [Auth API](/auth/how-auth-works/auth-api). To learn more about how to implement Auth on the client-side with Next.js, see the Auth [React page](/auth/client-frameworks/react) as Next uses React for the frontend:
...
| `cookieOptions.sameSite`             | The sameSite value set on all cookies. See [Mozilla cookie reference](https://developer.mozilla.org/en-US/docs/Web/HTTP/Cookies#samesite) for more information.                                                            |
