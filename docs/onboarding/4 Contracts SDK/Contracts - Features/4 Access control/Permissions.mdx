...
The `Permissions` and `PermissionsEnumerable` extensions are _not_ already included in any [base contracts](/extensions/base-contracts). To use this extension, check out the [implementation guide](/extensions/platformfee#implementing-the-contract-extension) and [full API reference](/extensions/permissions#full-api-reference).
