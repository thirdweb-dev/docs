...
# Drop Single Phase (ERC20 & ERC 721)
...
The `DropSinglePhase` extension is _already_ available in the following [base contracts](/extensions/base-contracts):

- **ERC20**: [`ERC20Drop`](/extensions/base-contracts/erc-20/erc20drop)
- **ERC721**: [`ERC721Drop`](/extensions/base-contracts/erc-721/erc721drop)
...
This section is meant for advanced users who want to write functionality from scratch. If that's not you, use one of these [base contracts](/extensions/dropsinglephase#availability-in-base-contracts) that already implement Drop functionlity for you!
...
| ------------------------------------------------------------------------------ | ----------------------- | ---------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| [`collectPriceOnClaim`](/extensions/dropsinglephase#collectpriceonclaim)       | internal _virtual_      | n/a        | n/a       | Collects and distributes the primary sale value of NFTs being claimed.                                                                           |
| [`transferTokensOnClaim`](/extensions/dropsinglephase#transfertokensonclaim)   | internal _virtual_      | n/a        | `uint256` | Transfers the NFTs being claimed.                                                                                                                |
| [`_canSetClaimConditions`](/extensions/dropsinglephase#_cansetclaimconditions) | internal view _virtual_ | n/a        | `bool`    | Runs on every attempt to set the claim condition on the contract. Returns whether the claim condition can be set in the given execution context. |
...
```solidity
function transferTokensOnClaim(address to, uint256 quantityBeingClaimed)
    internal
    virtual
