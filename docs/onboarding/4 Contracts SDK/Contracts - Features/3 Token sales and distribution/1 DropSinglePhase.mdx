...
- Parameter `phase`: The set of restrictions that will apply to the minting of tokens on the contract.
  ...
  - `waitTimeInSecondsBetweenClaims`: The least number of seconds an account must wait after claiming tokens, to be able to claim tokens again.
...
- Parameter `verifyMaxQuantityPerTransaction`: Whether to account for any allowlist criteria.
...
- Parameter `claimer`: The wallet claiming tokens.
- Parameter `quantity`: The quantity of tokens to claim.
- Parameter `allowlistProof`: The proof of the claimer's inclusion in the allowlist.

### getClaimTimestamp

```solidity
function getClaimTimestamp(address claimer)
    public
    view
