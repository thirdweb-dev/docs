---
slug: /contractkit/extension-contracts/batchmint
...
The `BatchMintMetadata` extension is _already_ available in the following [base contracts](/extensions/base-contracts):
...
```solidity
function getBatchId(uint256 tokenId) internal view returns (uint256 batchId, uint256 index);
```
