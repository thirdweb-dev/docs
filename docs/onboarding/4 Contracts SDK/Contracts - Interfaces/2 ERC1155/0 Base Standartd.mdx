---
slug: /contractkit/interfaces/erc1155
title: Standard
---

import ViewContractCodeButton from "@components/ViewContractCodeButton";
import QuickstartCard from "@components/QuickstartCard";

# ERC1155

ERC1155 is the standard for representing "Edition" NFTs
where each NFT in the collection can have more than 1 quantity; also known as "semi-fungible" tokens.

<ViewContractCodeButton
  link="https://github.com/thirdweb-dev/contracts/blob/main/contracts/eip/ERC1155.sol"
  name="ERC1155"
/>

## Base Contracts Implementing This Feature

<div className="row" style={{ marginBottom: 24 }}>
  <div className="col col--6" style={{ padding: 8 }}>
    <QuickstartCard
      name="ERC1155 Base"
      link="/contractkit/base-contracts/erc-1155/erc1155base"
      image="/assets/icons/general.png"
    />
  </div>
  <div className="col col--6" style={{ padding: 8 }}>
    <QuickstartCard
      name="ERC1155 Signature Mint"
      link="/contractkit/base-contracts/erc-1155/erc1155signaturemint"
      image="/assets/icons/general.png"
    />
  </div>
  <div className="col col--6" style={{ padding: 8 }}>
    <QuickstartCard
      name="ERC1155 Lazy Mint"
      link="/contractkit/base-contracts/erc-1155/erc1155lazymint"
      image="/assets/icons/general.png"
    />
  </div>
  <div className="col col--6" style={{ padding: 8 }}>
    <QuickstartCard
      name="ERC1155 Delayed Reveal"
      link="/contractkit/base-contracts/erc-1155/erc1155delayedreveal"
      image="/assets/icons/general.png"
    />
  </div>
  <div className="col col--6" style={{ padding: 8 }}>
    <QuickstartCard
      name="ERC1155 Drop"
      link="/contractkit/base-contracts/erc-1155/erc1155drop"
      image="/assets/icons/general.png"
    />
  </div>
</div>

## Unlocked Features

By implementing the `ERC1155` standard, you unlock the following features in the SDK and dashboard:

| SDK Feature                                        | Description                                                             |
| -------------------------------------------------- | ----------------------------------------------------------------------- |
| [Get NFT Balance](/typescript/sdk.erc1155#balance) | View the number of NFTs owned by a wallet address                       |
| [View NFT Metadata](/typescript/sdk.erc1155#get)   | View the token URI and fetch the metadata from IPFS                     |
| [Transfer](/typescript/sdk.erc1155#transfer)       | Transfer this NFT from one wallet to another                            |
| [Airdrop](/typescript/sdk.erc1155#airdrop)         | Airdrop one or multiple NFTs to the provided a list of wallet addresses |

## Implementing It Yourself

This section is meant for advanced users who want to write the functionality from scratch.

```solidity
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

import "@thirdweb-dev/contracts/eip/ERC1155.sol";

contract Contract is ERC1155 {
    constructor(
        string memory _name,
        string memory _symbol
    )
        ERC1155(
            _name,
            _symbol
        )
    {}
}
```
