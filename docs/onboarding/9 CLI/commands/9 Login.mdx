...
Once you've logged in, your deviced will be authorized for future use.

:::info
If you want to login with an API secret key, e.g. is your are using continuous integration, you can use the `--key` flag and pass a secret key.
:::
...
![Link Device to Account](/assets/dashboard/link-device-cli-auth.png)
...
- A [local wallet](/wallet/local-wallet) is created and stored in your device's local storage.
...
If you want to login with an API secret key, e.g. is your are using continuous integration (CI), you can use the `--key` flag and pass a secret key.
...
2. Click the `Acocunts` tab from the sidebar and find the device you would like to revoke.
3. Click the `Revoke Access` button to revoke access to the device.

![Revoke Access](/assets/dashboard/revoke-access-cli-auth.png)
