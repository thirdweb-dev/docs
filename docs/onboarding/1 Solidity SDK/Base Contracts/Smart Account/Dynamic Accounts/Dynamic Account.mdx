...
The difference between `DynamicAccount` smart wallet and [`Account`](/solidity/base-contracts/account) smart wallet is that `DynamicAccount` is an *upgradeable* smart contract written in the [dynamic contract pattern](https://github.com/thirdweb-dev/dynamic-contracts).
...
If you intend to issue accounts programmatically, using a factory contract you must do the following:

- Call `_disableInitializers()` at the end of the constructor.

- Create a new factory contract by extending the [BaseAccountFactory](/solidity/extensions/base-account-factory) extension

- Call `_initializeAccount` on the factory to create a new wallet.
