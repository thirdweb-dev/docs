...
You might notice your text editor asks you to mark the contract as `abstract`:

![abstract](./assets/abstract.png)

This is because our contract doesn't yet implement the functionality required to be an `IMintableERC721` contract.

You can `Ctrl / ⌘ + click` on `IMintableERC721` to see the functions that must be implemented to satisfy the interface!
In the gif below, you can see we need to implement the `mintTo` function.

![clickinto](./assets/clickinto.gif)

You have total freedom over how you choose to implement these functions in your contract, but let's explore an example of how you might implement it.
...
Firstly, we can copy the function signature from the `IMintableERC721` file by `Ctrl / ⌘ + click`ing on `IMintableERC721` again,
pasting it into our contract, and signifying we are going to `override` it.
...
## Detecting contract features

The thirdweb CLI provides a command to detect the features of a contract.
...
- Detects the features you have successfully implemented
- Shows you other features you might want to implement _(spoilers)_!
...
Let's go through the same process again, to demonstrate the general flow of adding features to your contract.
