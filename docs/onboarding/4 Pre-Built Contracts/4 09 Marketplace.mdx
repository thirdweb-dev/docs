...
- You list your NFT for direct sale.
- The NFT remains in your wallet.
- If a user pays the asking price, they will be transferred your NFT.
- Users can make offers below the asking price, and the listing can have multiple offers at once, you can choose to accept an offer at any time.
...
- You set the minimum asking price and users can place bids until the auction is finished.
- Users can make a bid if it is higher than the current highest bid. A bid cannot be cancelled, and the funds are held in escrow by the auction contract.
- At the end of the auction, no more bids can be placed.
- At the end of the auction, both the **buyer** and the **seller** call the `closeAuction` function, where the **buyer** is transferred the funds from the highest bid, and the **seller** is transferred the NFT.
...
We made a few important considerations with the auctions in our smart contract.

- When someone makes a bid in an auction, the time until the auction is finished is extended by a set amount you can configure. This is to avoid users not bidding until the last possible second and win the auction.
