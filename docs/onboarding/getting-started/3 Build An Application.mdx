...
This wrapper allows us to use all of the [React SDK](/react)'s hooks and [UI Components](/ui-components) throughout the application, as well as configure a `desiredChainId`;
...
[`useContractWrite`](/sdk/interacting-with-contracts/custom-contracts/using-contracts#write-transactions).
Each [extension](/contractkit/features) you implemented in your smart contract also
...
through the [`useNFTs`](/react/react.usenfts) hook, which we use by providing the `contract` object to it like so:
...
Use the [`Web3Button`](/ui-components/web3button) component to perform a `mint` transaction; ensuring the user
...
<TwitterCard
  title={"Share Your Build With Us On Twitter!"}
