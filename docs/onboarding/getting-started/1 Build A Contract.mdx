...
```bash
npx thirdweb@latest@latest create contract
```
...
- Select `None` for optional [extensions](/contractkit/extensions)
...
implements the [ERC721A](https://github.com/thirdweb-dev/contracts/blob/main/contracts/eip/ERC721A.sol) standard with several useful [extensions](/contractkit/extensions).

<h3 style={{ marginTop: 48 }}>Adding Extensions</h3>

[Extensions](/contractkit/extensions) are a great way to add individual pieces of
...
view its source code, see which [extensions](https://portal.thirdweb.com/contractkit/extensions) it implements,
...
```bash
npx thirdweb@latest@latest deploy
```
