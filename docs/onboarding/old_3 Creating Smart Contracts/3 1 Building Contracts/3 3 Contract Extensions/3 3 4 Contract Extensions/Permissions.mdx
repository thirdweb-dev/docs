...
# Permissions Contract Extensions

To add role-based restrictions on who can perform specific actions on your contract, there are two contract extensions available:

1. [`Permissions`](#permissions)
2. [`PermissionsEnumberable`](#permissionsenumberable) (extends `Permissions`)

If you need to view all members of a given role, the `PermissionsEnumberable` extension is required.
...
contract MyContract is PermissionsEnumberable {
    // Any `bytes32` value is a valid role. You can create roles by defining them like this.
    bytes32 public constant NUMBER_ROLE = keccak256("NUMBER_ROLE");
