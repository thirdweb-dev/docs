...
A **registry** is a way of storing all the versions of your code over time, in a way
that is publicly available to other users who might want to use it. For example,
each new release of our [React SDK](https://www.npmjs.com/package/@thirdweb-dev/react) is
available for our community to install via `npm`.

These pages are accessible by anyone and allow users who visit them to:

- View the author and the source of the contract
- View the functions, variables, and [Extensions](/extensions) in your contract
