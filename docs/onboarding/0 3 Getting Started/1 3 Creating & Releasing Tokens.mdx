...
Drop contracts such as the [NFT Drop](/pre-built-contracts/nft-drop) allow you to [lazy mint](/pre-built-contracts/nft-drop#lazy-minting-your-nfts) tokens
and release them to be claimed by other wallets.
They include configurable [claim phases](/pre-built-contracts/nft-drop#setting-claim-phases) that define the conditions under which a token can be claimed,
such as an allowlist, price, and release date.
...
All of the metadata you configure will automatically be [pinned to IPFS](https://blog.thirdweb.com/guides/securing-pinning-your-nft-with-ipfs).
