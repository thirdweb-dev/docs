...
On the `index.tsx` page, we are using the [useActiveListings](/react/react.useactivelistings) hook to fetch all active listings from the smart contract.
...
You can see we are initializing the **marketplace contract** using the [useMarketplace](/react/react.usemarketplace) hook and passing the contract as a parameter to the `useActiveListings`.
