...
It loads all of a user's owned tokens by making an API request to the [Get token balances for address](https://www.covalenthq.com/docs/api/#/0/Get%20token%20balances%20for%20address/USD/1) endpoint. Using thirdweb's React SDK, we fetch the metadata about each ERC721 NFT and display it in a list using the
...
Firstly, we map over the different kinds of tokens returned from the API endpoint, filtering to ERC721 NFTs.
