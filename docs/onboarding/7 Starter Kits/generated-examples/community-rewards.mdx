...
This works because we have the `ThirdwebProvider` setup in our [\_app.js](https://github.com/thirdweb-example/community-rewards/blob/main/pages/_app.js) file, which allows us to use all of the thirdweb React SDK's helpful hooks.
...
`NextAuth` uses the [`pages/api/auth/[...nextauth].js`](https://github.com/thirdweb-example/community-rewards/blob/main/pages/api/auth/[...nextauth].js) file to handle the authentication logic such as redirects for us.
...
This logic is performed on the [pages/api/check-is-in-server.js](https://github.com/thirdweb-example/community-rewards/blob/main/pages/api/check-is-in-server.js) file.
