...
We deploy a basic [HelloWorld](https://github.com/thirdweb-example/thirdweb-deploy-hello-world/blob/main/HelloWorld.sol) smart contract using thirdweb deploy, containing a `get` and `set` function to view data stored in the smart contract, and interact with it using the [thirdweb React SDK](https://portal.thirdweb.com/react).
