...
Each individual session key has their own permissions to use the account. Only admins can set the permissions for session keys.

Session keys can be assigned the following permissions:

- Allow interacting with specific contracts with the account.
...
The easiest way to manage session keys is using the Wallet SDK:
