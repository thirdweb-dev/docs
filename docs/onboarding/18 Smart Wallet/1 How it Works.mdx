...
This is because, when the account contract is cloned by the factory, the account is created deterministically
...
A bundler is a node that monitors the alternative mempool of `UserOperations` and bundles multiple `UpserOps` together to forward to the EntryPoint contract as a single transaction.
