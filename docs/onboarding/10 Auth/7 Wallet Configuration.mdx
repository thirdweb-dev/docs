...
Given that private key and signer wallets for EVM and Solana depend on different packages underneath, they are accessible via separate `@thirdweb-dev/auth/evm` and `@thirdweb-dev/auth/solana` entry points, which each have their own `peerDependencies`.
