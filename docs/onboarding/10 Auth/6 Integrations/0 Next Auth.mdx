...
Now, we can configure the `ThirdwebAuthProvider` on our [NextAuth API routes](https://next-auth.js.org/getting-started/example#add-api-route) to add support for wallet based login, in addition to all other existing authentication methods. The `ThirdwebAuthProvider` uses NextAuth's [Credentials Provider](https://next-auth.js.org/configuration/providers/credentials) underneath to enable custom wallet based authentication (for those curious to dig deeper, you can take a look at the [`ThirdwebAuthProvider` implementation](https://github.com/thirdweb-dev/js/blob/b5c6eedb38aa3c52eb97f3d25ad83e38c55afe61/packages/auth/src/next-auth/index.ts#L9)).
...
NextAuth's [`getServerSession` function](https://next-auth.js.org/configuration/nextjs#getserversession) can be used to authenticate the user on the server. It will return the active session data if the user is logged in, or `null` if the user is not authenticated.
