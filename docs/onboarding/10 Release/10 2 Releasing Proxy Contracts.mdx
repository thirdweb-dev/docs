...
When releasing a contract, you can choose to let others deploy a "proxy" of a pre-deployed contract. Since proxies are much lighter weight than the full contract, they are much cheaper to deploy. Proxy deploys only require a fraction of the gas of a normal deploy.
...
To release a proxy contract, your contract needs to implement the [upgradeable pattern](https://docs.openzeppelin.com/upgrades-plugins/1.x/writing-upgradeable), which it has an "initialize" function instead of a regular constructor.
...
![Proxy Contract Parameters](/assets/publish/contract-parameters.png)
