...
Wallet SDK comes out of the box with support for the most popular wallets that are ready to be used with thirdweb SDKs:
...
When you want to use a wallet with React/React Native SDK - Instead of importing the wallet from `@thirdweb-dev/wallets` , you need to import a “wallet configurator” function from the `@thirdweb-dev/react` or `@thirdweb-dev/react-native`. This wallet configurator function contains additional metadata and UI for connecting the wallet which is shown in the [ConnectWallet](/react/react.connectwallet) component's Modal.
...
[useConnect](/react/react.useconnect) hook allows you to programmatically connect to the wallet if you do not want to use the [ConnectWallet](/react/react.connectwallet) component.
...
thirdweb's Unity SDK provides a [ConnectWallet](/unity/connectwallet) prefab that allows users to connect their wallet to your game. [Learn more about Unity SDK](/unity)
