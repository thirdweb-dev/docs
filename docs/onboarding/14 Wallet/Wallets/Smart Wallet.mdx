...
The API key let's you access thirdweb's bundler and paymaster infrastructure, which is required for the smart wallet to operate and optionally enable [gasless transactions](/glossary/gasless-transactions).
...
Once instantiated, you can use the following methods to interact with the wallet.
...
The wallet also inherits the following methods from the `AbstractClientWallet` class.
