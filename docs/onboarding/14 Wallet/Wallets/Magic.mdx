...
There are two ways to call the `connect` method - with email or phoneNumber

If you call the `connect` method with email, a modal will popup and prompt the to check their mail and click on the link sent to their email. Once user completes this step, the modal will close and the user will be connected to your app.
...
If you call the `connect` method with phoneNumber, it will send an OTP to the user's phone number. Once the user enters the OTP, the modal will close and the user will be connected to your app.
...
Returns a `string` containing the wallet address, or throws an error if connection failed.
...
Disconnect the currently connected wallet from your app and log out the user from magic link. Once logged out - autoconnect will not work.
...
Add a listener for a specific event, and run it whenever that event is emitted.
...
User changes the account they are connected with, or the chain they are connected to.
...
It only works if the user has already connected to the wallet to your app before.
...
Returns a `string` containing the wallet address, or throws an error if wallet can not be autoconnected
...
```javascript
wallet.removeAllListeners(); // for all events
wallet.removeAllListeners(eventName); // for a specific event
```
...
Remove a specific event listener (callback) for a given event.
...
```javascript
const isValid = await wallet.verifySignature(message, signature, address, chainId?);
```
