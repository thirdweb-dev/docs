...
The API key will need to have the following:
- smart wallet as an enabled service
- The contracts that the smart wallet will interact with must be whitelisted. Learn more [here](/api-keys#smart-wallet-allowed-destination-addresses).
:::
...
<details>
  <summary>clientId or secretKey (recommend)</summary>
  <div>
