...
=======
## Extending the `TWConnector` class

The [`TWConnector`](https://github.com/thirdweb-dev/js/blob/main/packages/wallets/src/evm/interfaces/tw-connector.ts) abstract class is the
>>>>>>> main
...
=======
Note that the `TWConnector` extends the `EventEmitter` class from the `events` package. This way you can emit events from your wallet.
>>>>>>> main
...
=======
that resolves to a the [`TWConnector`](https://github.com/thirdweb-dev/js/blob/main/packages/wallets/src/evm/interfaces/tw-connector.ts#L6)
>>>>>>> main
...
=======
Returns a `Promise<TWConnector>` with the connector class that you implemented.
>>>>>>> main
