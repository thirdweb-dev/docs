...
The last step is to integrate it with [Connect Wallet](/react/react.connectwallet) button by creating a wallet configurator - a function that returns a `WalletConfig` object and specifying it in [supportedWallets](/react/react.thirdwebprovider#supportedwallets-optional)
...
<img src="/assets/connect-wallet/connecting-metamask.png" />
...
```ts
{
   ...
   *
   * Geting the entire `WalletConfig` object is useful for connecting the wallet and getting metadata
   */
   ...
   * If only one wallet (your wallet) is supported, Connect Wallet Modal will skip showing the wallet selection screen and directly render the connectUI of your wallet.
   *
