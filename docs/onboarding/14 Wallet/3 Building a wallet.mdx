...
The last step is to integrate your wallet with [Connect Wallet](/react/react.connectwallet) button. This is done by creating a function that returns a `ConfiguredWallet` object.

See the Detailed Guide for [Integrating with Connect Wallet](/wallet/build-wallet-ui) for more info

```ts
const myWallet = (config: MyConfig): ConfiguredWallet => {
  return {
