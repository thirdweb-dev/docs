...
# Building contracts and apps on Syscoin Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Syscoin Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on Syscoin Mainnet. If you want to check details such as RPCs for Syscoin Mainnet, check out our [chainlist page for Syscoin Mainnet](https://thirdweb.com/syscoin).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Syscoin Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Syscoin Mainnet as the network
...
### Initialize SDK On Syscoin Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Syscoin Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Syscoin } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Syscoin}>
      <YourApp />
    </ThirdwebProvider>
  );
};
