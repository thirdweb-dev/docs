...
# Building contracts and apps on Gear Zero Network Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Gear Zero Network Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Gear Zero Network Testnet. If you want to check details such as RPCs for Gear Zero Network Testnet, check out our [chainlist page for Gear Zero Network Testnet](https://thirdweb.com/gear-zero-network-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Gear Zero Network Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Gear Zero Network Testnet as the network
...
### Initialize SDK On Gear Zero Network Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Gear Zero Network Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { GearZeroNetworkTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={GearZeroNetworkTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
