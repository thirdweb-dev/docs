...
# Building contracts and apps on POA Network Core

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on POA Network Core. By the end of the guide, you should be able to create smart contracts and dApps on POA Network Core. If you want to check details such as RPCs for POA Network Core, check out our [chainlist page for POA Network Core](https://thirdweb.com/poa-network-core).
...
5. After modifying your contract with your desired custom logic, you may deploy it to POA Network Core using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select POA Network Core as the network
...
### Initialize SDK On POA Network Core

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to POA Network Core

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { PoaNetworkCore } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={PoaNetworkCore}>
      <YourApp />
    </ThirdwebProvider>
  );
};
