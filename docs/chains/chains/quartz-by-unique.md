...
# Building contracts and apps on Quartz by Unique

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Quartz by Unique. By the end of the guide, you should be able to create smart contracts and dApps on Quartz by Unique. If you want to check details such as RPCs for Quartz by Unique, check out our [chainlist page for Quartz by Unique](https://thirdweb.com/quartz-by-unique).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Quartz by Unique using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Quartz by Unique as the network
...
### Initialize SDK On Quartz by Unique

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Quartz by Unique

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { QuartzByUnique } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={QuartzByUnique}>
      <YourApp />
    </ThirdwebProvider>
  );
};
