...
# Building contracts and apps on ETND Chain Mainnets

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on ETND Chain Mainnets. By the end of the guide, you should be able to create smart contracts and dApps on ETND Chain Mainnets. If you want to check details such as RPCs for ETND Chain Mainnets, check out our [chainlist page for ETND Chain Mainnets](https://thirdweb.com/etnd-chain-s).
...
5. After modifying your contract with your desired custom logic, you may deploy it to ETND Chain Mainnets using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select ETND Chain Mainnets as the network
...
### Initialize SDK On ETND Chain Mainnets

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to ETND Chain Mainnets

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { EtndChainS } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={EtndChainS}>
      <YourApp />
    </ThirdwebProvider>
  );
};
