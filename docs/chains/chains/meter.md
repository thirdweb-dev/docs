...
5. After modifying your contract with your desired custom logic, you may deploy it to Meter Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Meter Mainnet as the network
...
### Initialize SDK On Meter Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Meter Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Meter } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Meter}>
      <YourApp />
    </ThirdwebProvider>
  );
};
