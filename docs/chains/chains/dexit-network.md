...
# Building contracts and apps on Dexit Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Dexit Network. By the end of the guide, you should be able to create smart contracts and dApps on Dexit Network. If you want to check details such as RPCs for Dexit Network, check out our [chainlist page for Dexit Network](https://thirdweb.com/dexit-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Dexit Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Dexit Network as the network
...
### Initialize SDK On Dexit Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Dexit Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { DexitNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={DexitNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
