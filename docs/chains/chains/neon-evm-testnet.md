...
# Building contracts and apps on Neon EVM TestNet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Neon EVM TestNet. By the end of the guide, you should be able to create smart contracts and dApps on Neon EVM TestNet. If you want to check details such as RPCs for Neon EVM TestNet, check out our [chainlist page for Neon EVM TestNet](https://thirdweb.com/neon-evm-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Neon EVM TestNet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Neon EVM TestNet as the network
...
### Initialize SDK On Neon EVM TestNet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Neon EVM TestNet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { NeonEvmTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={NeonEvmTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
