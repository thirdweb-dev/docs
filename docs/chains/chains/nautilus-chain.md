...
# Building contracts and apps on Nautilus Chain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Nautilus Chain. By the end of the guide, you should be able to create smart contracts and dApps on Nautilus Chain. If you want to check details such as RPCs for Nautilus Chain, check out our [chainlist page for Nautilus Chain](https://thirdweb.com/nautilus-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Nautilus Chain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Nautilus Chain as the network
...
### Initialize SDK On Nautilus Chain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Nautilus Chain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { NautilusChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={NautilusChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
