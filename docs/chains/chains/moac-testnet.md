...
# Building contracts and apps on MOAC testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on MOAC testnet. By the end of the guide, you should be able to create smart contracts and dApps on MOAC testnet. If you want to check details such as RPCs for MOAC testnet, check out our [chainlist page for MOAC testnet](https://thirdweb.com/moac-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to MOAC testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select MOAC testnet as the network
...
### Initialize SDK On MOAC testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to MOAC testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { MoacTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={MoacTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
