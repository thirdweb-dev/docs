...
# Building contracts and apps on Metal C-Chain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Metal C-Chain. By the end of the guide, you should be able to create smart contracts and dApps on Metal C-Chain. If you want to check details such as RPCs for Metal C-Chain, check out our [chainlist page for Metal C-Chain](https://thirdweb.com/metal-c-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Metal C-Chain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Metal C-Chain as the network
...
### Initialize SDK On Metal C-Chain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Metal C-Chain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { MetalCChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={MetalCChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
