...
# Building contracts and apps on DFK Chain Test

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on DFK Chain Test. By the end of the guide, you should be able to create smart contracts and dApps on DFK Chain Test. If you want to check details such as RPCs for DFK Chain Test, check out our [chainlist page for DFK Chain Test](https://thirdweb.com/dfk-chain-test).
...
5. After modifying your contract with your desired custom logic, you may deploy it to DFK Chain Test using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select DFK Chain Test as the network
...
### Initialize SDK On DFK Chain Test

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to DFK Chain Test

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { DfkChainTest } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={DfkChainTest}>
      <YourApp />
    </ThirdwebProvider>
  );
};
