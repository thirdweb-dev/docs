...
# Building contracts and apps on QuarkChain Mainnet Root

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on QuarkChain Mainnet Root. By the end of the guide, you should be able to create smart contracts and dApps on QuarkChain Mainnet Root. If you want to check details such as RPCs for QuarkChain Mainnet Root, check out our [chainlist page for QuarkChain Mainnet Root](https://thirdweb.com/quarkchain-root).
...
5. After modifying your contract with your desired custom logic, you may deploy it to QuarkChain Mainnet Root using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select QuarkChain Mainnet Root as the network
...
### Initialize SDK On QuarkChain Mainnet Root

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to QuarkChain Mainnet Root

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { QuarkchainRoot } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={QuarkchainRoot}>
      <YourApp />
    </ThirdwebProvider>
  );
};
