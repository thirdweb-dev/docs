...
# Building contracts and apps on YuanChain Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on YuanChain Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on YuanChain Mainnet. If you want to check details such as RPCs for YuanChain Mainnet, check out our [chainlist page for YuanChain Mainnet](https://thirdweb.com/yuanchain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to YuanChain Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select YuanChain Mainnet as the network
...
### Initialize SDK On YuanChain Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to YuanChain Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Yuanchain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Yuanchain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
