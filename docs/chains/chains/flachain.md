...
# Building contracts and apps on Flachain Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Flachain Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on Flachain Mainnet. If you want to check details such as RPCs for Flachain Mainnet, check out our [chainlist page for Flachain Mainnet](https://thirdweb.com/flachain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Flachain Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Flachain Mainnet as the network
...
### Initialize SDK On Flachain Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Flachain Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Flachain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Flachain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
