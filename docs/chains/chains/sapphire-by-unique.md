...
# Building contracts and apps on Sapphire by Unique

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Sapphire by Unique. By the end of the guide, you should be able to create smart contracts and dApps on Sapphire by Unique. If you want to check details such as RPCs for Sapphire by Unique, check out our [chainlist page for Sapphire by Unique](https://thirdweb.com/sapphire-by-unique).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Sapphire by Unique using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Sapphire by Unique as the network
...
### Initialize SDK On Sapphire by Unique

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Sapphire by Unique

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { SapphireByUnique } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={SapphireByUnique}>
      <YourApp />
    </ThirdwebProvider>
  );
};
