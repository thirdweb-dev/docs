...
# Building contracts and apps on F(x)Core Mainnet Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on F(x)Core Mainnet Network. By the end of the guide, you should be able to create smart contracts and dApps on F(x)Core Mainnet Network. If you want to check details such as RPCs for F(x)Core Mainnet Network, check out our [chainlist page for F(x)Core Mainnet Network](https://thirdweb.com/f-x-core-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to F(x)Core Mainnet Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select F(x)Core Mainnet Network as the network
...
### Initialize SDK On F(x)Core Mainnet Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to F(x)Core Mainnet Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { FXCoreNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={FXCoreNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
