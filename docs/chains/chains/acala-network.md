...
# Building contracts and apps on Acala Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Acala Network. By the end of the guide, you should be able to create smart contracts and dApps on Acala Network. If you want to check details such as RPCs for Acala Network, check out our [chainlist page for Acala Network](https://thirdweb.com/acala-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Acala Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Acala Network as the network
...
### Initialize SDK On Acala Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Acala Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { AcalaNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={AcalaNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
