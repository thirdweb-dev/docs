...
# Building contracts and apps on Razor Skale Chain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Razor Skale Chain. By the end of the guide, you should be able to create smart contracts and dApps on Razor Skale Chain. If you want to check details such as RPCs for Razor Skale Chain, check out our [chainlist page for Razor Skale Chain](https://thirdweb.com/razor-skale-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Razor Skale Chain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Razor Skale Chain as the network
...
### Initialize SDK On Razor Skale Chain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Razor Skale Chain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { RazorSkaleChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={RazorSkaleChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
