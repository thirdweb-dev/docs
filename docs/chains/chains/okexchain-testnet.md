...
# Building contracts and apps on OKExChain Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on OKExChain Testnet. By the end of the guide, you should be able to create smart contracts and dApps on OKExChain Testnet. If you want to check details such as RPCs for OKExChain Testnet, check out our [chainlist page for OKExChain Testnet](https://thirdweb.com/okexchain-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to OKExChain Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select OKExChain Testnet as the network
...
### Initialize SDK On OKExChain Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to OKExChain Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { OkexchainTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={OkexchainTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
