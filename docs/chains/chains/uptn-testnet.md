...
# Building contracts and apps on UPTN Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on UPTN Testnet. By the end of the guide, you should be able to create smart contracts and dApps on UPTN Testnet. If you want to check details such as RPCs for UPTN Testnet, check out our [chainlist page for UPTN Testnet](https://thirdweb.com/uptn-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to UPTN Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select UPTN Testnet as the network
...
### Initialize SDK On UPTN Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to UPTN Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { UptnTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={UptnTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
