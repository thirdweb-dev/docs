...
# Building contracts and apps on VChain Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on VChain Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on VChain Mainnet. If you want to check details such as RPCs for VChain Mainnet, check out our [chainlist page for VChain Mainnet](https://thirdweb.com/vchain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to VChain Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select VChain Mainnet as the network
...
### Initialize SDK On VChain Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to VChain Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Vchain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Vchain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
