...
# Building contracts and apps on Zeeth Chain Dev

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Zeeth Chain Dev. By the end of the guide, you should be able to create smart contracts and dApps on Zeeth Chain Dev. If you want to check details such as RPCs for Zeeth Chain Dev, check out our [chainlist page for Zeeth Chain Dev](https://thirdweb.com/zeeth-chain-dev).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Zeeth Chain Dev using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Zeeth Chain Dev as the network
...
### Initialize SDK On Zeeth Chain Dev

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Zeeth Chain Dev

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ZeethChainDev } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ZeethChainDev}>
      <YourApp />
    </ThirdwebProvider>
  );
};
