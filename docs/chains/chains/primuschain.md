...
# Building contracts and apps on PrimusChain mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on PrimusChain mainnet. By the end of the guide, you should be able to create smart contracts and dApps on PrimusChain mainnet. If you want to check details such as RPCs for PrimusChain mainnet, check out our [chainlist page for PrimusChain mainnet](https://thirdweb.com/primuschain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to PrimusChain mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select PrimusChain mainnet as the network
...
### Initialize SDK On PrimusChain mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to PrimusChain mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Primuschain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Primuschain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
