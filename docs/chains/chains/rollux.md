...
# Building contracts and apps on Rollux Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Rollux Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on Rollux Mainnet. If you want to check details such as RPCs for Rollux Mainnet, check out our [chainlist page for Rollux Mainnet](https://thirdweb.com/rollux).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Rollux Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Rollux Mainnet as the network
...
### Initialize SDK On Rollux Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Rollux Mainnet
...
const App = () => {
  return (
    <ThirdwebProvider activeChain={Rollux}>
      <YourApp />
    </ThirdwebProvider>
  );
};
