...
# Building contracts and apps on Cube Chain Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Cube Chain Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on Cube Chain Mainnet. If you want to check details such as RPCs for Cube Chain Mainnet, check out our [chainlist page for Cube Chain Mainnet](https://thirdweb.com/cube-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Cube Chain Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Cube Chain Mainnet as the network
...
### Initialize SDK On Cube Chain Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Cube Chain Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { CubeChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={CubeChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
