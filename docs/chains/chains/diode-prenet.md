...
# Building contracts and apps on Diode Prenet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Diode Prenet. By the end of the guide, you should be able to create smart contracts and dApps on Diode Prenet. If you want to check details such as RPCs for Diode Prenet, check out our [chainlist page for Diode Prenet](https://thirdweb.com/diode-prenet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Diode Prenet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Diode Prenet as the network
...
### Initialize SDK On Diode Prenet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Diode Prenet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { DiodePrenet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={DiodePrenet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
