...
5. After modifying your contract with your desired custom logic, you may deploy it to BlackFort Exchange Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select BlackFort Exchange Network as the network
...
### Initialize SDK On BlackFort Exchange Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to BlackFort Exchange Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { BlackfortExchangeNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={BlackfortExchangeNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
