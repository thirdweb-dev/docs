...
# Building contracts and apps on Velas EVM Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Velas EVM Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on Velas EVM Mainnet. If you want to check details such as RPCs for Velas EVM Mainnet, check out our [chainlist page for Velas EVM Mainnet](https://thirdweb.com/velas-evm).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Velas EVM Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Velas EVM Mainnet as the network
...
### Initialize SDK On Velas EVM Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Velas EVM Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { VelasEvm } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={VelasEvm}>
      <YourApp />
    </ThirdwebProvider>
  );
};
