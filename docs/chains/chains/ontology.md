...
# Building contracts and apps on Ontology Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Ontology Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on Ontology Mainnet. If you want to check details such as RPCs for Ontology Mainnet, check out our [chainlist page for Ontology Mainnet](https://thirdweb.com/ontology).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Ontology Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Ontology Mainnet as the network
...
### Initialize SDK On Ontology Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Ontology Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Ontology } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Ontology}>
      <YourApp />
    </ThirdwebProvider>
  );
};
