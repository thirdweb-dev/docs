...
# Building contracts and apps on OpenChain Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on OpenChain Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on OpenChain Mainnet. If you want to check details such as RPCs for OpenChain Mainnet, check out our [chainlist page for OpenChain Mainnet](https://thirdweb.com/openchain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to OpenChain Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select OpenChain Mainnet as the network
...
### Initialize SDK On OpenChain Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to OpenChain Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Openchain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Openchain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
