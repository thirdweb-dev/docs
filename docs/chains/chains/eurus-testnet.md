...
# Building contracts and apps on Eurus Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Eurus Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Eurus Testnet. If you want to check details such as RPCs for Eurus Testnet, check out our [chainlist page for Eurus Testnet](https://thirdweb.com/eurus-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Eurus Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Eurus Testnet as the network
...
### Initialize SDK On Eurus Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Eurus Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { EurusTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={EurusTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
