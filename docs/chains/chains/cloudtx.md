...
# Building contracts and apps on CloudTx Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on CloudTx Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on CloudTx Mainnet. If you want to check details such as RPCs for CloudTx Mainnet, check out our [chainlist page for CloudTx Mainnet](https://thirdweb.com/cloudtx).
...
5. After modifying your contract with your desired custom logic, you may deploy it to CloudTx Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select CloudTx Mainnet as the network
...
### Initialize SDK On CloudTx Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to CloudTx Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Cloudtx } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Cloudtx}>
      <YourApp />
    </ThirdwebProvider>
  );
};
