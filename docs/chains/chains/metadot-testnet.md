...
# Building contracts and apps on MetaDot Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on MetaDot Testnet. By the end of the guide, you should be able to create smart contracts and dApps on MetaDot Testnet. If you want to check details such as RPCs for MetaDot Testnet, check out our [chainlist page for MetaDot Testnet](https://thirdweb.com/metadot-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to MetaDot Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select MetaDot Testnet as the network
...
### Initialize SDK On MetaDot Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to MetaDot Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { MetadotTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={MetadotTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
