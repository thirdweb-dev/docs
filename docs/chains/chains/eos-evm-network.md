...
# Building contracts and apps on EOS EVM Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on EOS EVM Network. By the end of the guide, you should be able to create smart contracts and dApps on EOS EVM Network. If you want to check details such as RPCs for EOS EVM Network, check out our [chainlist page for EOS EVM Network](https://thirdweb.com/eos-evm-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to EOS EVM Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select EOS EVM Network as the network
...
### Initialize SDK On EOS EVM Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to EOS EVM Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { EosEvmNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={EosEvmNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
