...
# Building contracts and apps on myOwn Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on myOwn Testnet. By the end of the guide, you should be able to create smart contracts and dApps on myOwn Testnet. If you want to check details such as RPCs for myOwn Testnet, check out our [chainlist page for myOwn Testnet](https://thirdweb.com/myown-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to myOwn Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select myOwn Testnet as the network
...
### Initialize SDK On myOwn Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to myOwn Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { MyownTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={MyownTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
