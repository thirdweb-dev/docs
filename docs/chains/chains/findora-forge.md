...
# Building contracts and apps on Findora Forge

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Findora Forge. By the end of the guide, you should be able to create smart contracts and dApps on Findora Forge. If you want to check details such as RPCs for Findora Forge, check out our [chainlist page for Findora Forge](https://thirdweb.com/findora-forge).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Findora Forge using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Findora Forge as the network
...
### Initialize SDK On Findora Forge

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Findora Forge

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { FindoraForge } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={FindoraForge}>
      <YourApp />
    </ThirdwebProvider>
  );
};
