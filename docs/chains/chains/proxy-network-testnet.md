...
# Building contracts and apps on Proxy Network Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Proxy Network Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Proxy Network Testnet. If you want to check details such as RPCs for Proxy Network Testnet, check out our [chainlist page for Proxy Network Testnet](https://thirdweb.com/proxy-network-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Proxy Network Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Proxy Network Testnet as the network
...
### Initialize SDK On Proxy Network Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Proxy Network Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ProxyNetworkTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ProxyNetworkTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
