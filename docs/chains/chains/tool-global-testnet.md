...
# Building contracts and apps on TOOL Global Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on TOOL Global Testnet. By the end of the guide, you should be able to create smart contracts and dApps on TOOL Global Testnet. If you want to check details such as RPCs for TOOL Global Testnet, check out our [chainlist page for TOOL Global Testnet](https://thirdweb.com/tool-global-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to TOOL Global Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select TOOL Global Testnet as the network
...
### Initialize SDK On TOOL Global Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to TOOL Global Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ToolGlobalTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ToolGlobalTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
