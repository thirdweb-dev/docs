...
# Building contracts and apps on IVAR Chain Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on IVAR Chain Testnet. By the end of the guide, you should be able to create smart contracts and dApps on IVAR Chain Testnet. If you want to check details such as RPCs for IVAR Chain Testnet, check out our [chainlist page for IVAR Chain Testnet](https://thirdweb.com/ivar-chain-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to IVAR Chain Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select IVAR Chain Testnet as the network
...
### Initialize SDK On IVAR Chain Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to IVAR Chain Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { IvarChainTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={IvarChainTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
