...
# Building contracts and apps on SX Network Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on SX Network Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on SX Network Mainnet. If you want to check details such as RPCs for SX Network Mainnet, check out our [chainlist page for SX Network Mainnet](https://thirdweb.com/sx-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to SX Network Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select SX Network Mainnet as the network
...
### Initialize SDK On SX Network Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to SX Network Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { SxNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={SxNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
