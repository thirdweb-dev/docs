...
# Building contracts and apps on Conflux eSpace (Testnet)

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Conflux eSpace (Testnet). By the end of the guide, you should be able to create smart contracts and dApps on Conflux eSpace (Testnet). If you want to check details such as RPCs for Conflux eSpace (Testnet), check out our [chainlist page for Conflux eSpace (Testnet)](https://thirdweb.com/conflux-espace-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Conflux eSpace (Testnet) using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Conflux eSpace (Testnet) as the network
...
### Initialize SDK On Conflux eSpace (Testnet)

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Conflux eSpace (Testnet)
...
const App = () => {
  return (
    <ThirdwebProvider activeChain={ConfluxEspaceTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
