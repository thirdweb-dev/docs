...
# Building contracts and apps on Crypto Emergency

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Crypto Emergency. By the end of the guide, you should be able to create smart contracts and dApps on Crypto Emergency. If you want to check details such as RPCs for Crypto Emergency, check out our [chainlist page for Crypto Emergency](https://thirdweb.com/crypto-emergency).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Crypto Emergency using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Crypto Emergency as the network
...
### Initialize SDK On Crypto Emergency

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Crypto Emergency

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { CryptoEmergency } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={CryptoEmergency}>
      <YourApp />
    </ThirdwebProvider>
  );
};
