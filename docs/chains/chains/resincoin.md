...
# Building contracts and apps on ResinCoin Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on ResinCoin Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on ResinCoin Mainnet. If you want to check details such as RPCs for ResinCoin Mainnet, check out our [chainlist page for ResinCoin Mainnet](https://thirdweb.com/resincoin).
...
5. After modifying your contract with your desired custom logic, you may deploy it to ResinCoin Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select ResinCoin Mainnet as the network
...
### Initialize SDK On ResinCoin Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to ResinCoin Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Resincoin } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Resincoin}>
      <YourApp />
    </ThirdwebProvider>
  );
};
