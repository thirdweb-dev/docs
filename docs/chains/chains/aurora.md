...
5. After modifying your contract with your desired custom logic, you may deploy it to Aurora Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Aurora Mainnet as the network
...
### Initialize SDK On Aurora Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Aurora Mainnet
...
const App = () => {
  return (
    <ThirdwebProvider activeChain={Aurora}>
      <YourApp />
    </ThirdwebProvider>
  );
};
