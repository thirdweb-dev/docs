...
# Building contracts and apps on ZCore Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on ZCore Testnet. By the end of the guide, you should be able to create smart contracts and dApps on ZCore Testnet. If you want to check details such as RPCs for ZCore Testnet, check out our [chainlist page for ZCore Testnet](https://thirdweb.com/zcore-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to ZCore Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select ZCore Testnet as the network
...
### Initialize SDK On ZCore Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to ZCore Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ZcoreTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ZcoreTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
