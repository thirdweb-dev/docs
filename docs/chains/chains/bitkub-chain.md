...
# Building contracts and apps on Bitkub Chain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Bitkub Chain. By the end of the guide, you should be able to create smart contracts and dApps on Bitkub Chain. If you want to check details such as RPCs for Bitkub Chain, check out our [chainlist page for Bitkub Chain](https://thirdweb.com/bitkub-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Bitkub Chain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Bitkub Chain as the network
...
### Initialize SDK On Bitkub Chain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Bitkub Chain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { BitkubChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={BitkubChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
