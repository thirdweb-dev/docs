...
# Building contracts and apps on REI Chain Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on REI Chain Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on REI Chain Mainnet. If you want to check details such as RPCs for REI Chain Mainnet, check out our [chainlist page for REI Chain Mainnet](https://thirdweb.com/rei-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to REI Chain Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select REI Chain Mainnet as the network
...
### Initialize SDK On REI Chain Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to REI Chain Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ReiChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ReiChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
