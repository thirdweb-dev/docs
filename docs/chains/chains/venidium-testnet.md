...
# Building contracts and apps on Venidium Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Venidium Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Venidium Testnet. If you want to check details such as RPCs for Venidium Testnet, check out our [chainlist page for Venidium Testnet](https://thirdweb.com/venidium-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Venidium Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Venidium Testnet as the network
...
### Initialize SDK On Venidium Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Venidium Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { VenidiumTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={VenidiumTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
