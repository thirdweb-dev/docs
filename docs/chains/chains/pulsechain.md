...
# Building contracts and apps on PulseChain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on PulseChain. By the end of the guide, you should be able to create smart contracts and dApps on PulseChain. If you want to check details such as RPCs for PulseChain, check out our [chainlist page for PulseChain](https://thirdweb.com/pulsechain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to PulseChain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select PulseChain as the network
...
### Initialize SDK On PulseChain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to PulseChain
...
const App = () => {
  return (
    <ThirdwebProvider activeChain={Pulsechain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
