...
# Building contracts and apps on Taycan Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Taycan Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Taycan Testnet. If you want to check details such as RPCs for Taycan Testnet, check out our [chainlist page for Taycan Testnet](https://thirdweb.com/taycan-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Taycan Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Taycan Testnet as the network
...
### Initialize SDK On Taycan Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Taycan Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { TaycanTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={TaycanTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
