...
# Building contracts and apps on MainnetZ Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on MainnetZ Testnet. By the end of the guide, you should be able to create smart contracts and dApps on MainnetZ Testnet. If you want to check details such as RPCs for MainnetZ Testnet, check out our [chainlist page for MainnetZ Testnet](https://thirdweb.com/z-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to MainnetZ Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select MainnetZ Testnet as the network
...
### Initialize SDK On MainnetZ Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to MainnetZ Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ZTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ZTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
