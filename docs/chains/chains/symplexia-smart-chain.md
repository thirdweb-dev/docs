...
# Building contracts and apps on Symplexia Smart Chain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Symplexia Smart Chain. By the end of the guide, you should be able to create smart contracts and dApps on Symplexia Smart Chain. If you want to check details such as RPCs for Symplexia Smart Chain, check out our [chainlist page for Symplexia Smart Chain](https://thirdweb.com/symplexia-smart-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Symplexia Smart Chain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Symplexia Smart Chain as the network
...
### Initialize SDK On Symplexia Smart Chain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Symplexia Smart Chain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { SymplexiaSmartChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={SymplexiaSmartChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
