...
# Building contracts and apps on SoterOne Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on SoterOne Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on SoterOne Mainnet. If you want to check details such as RPCs for SoterOne Mainnet, check out our [chainlist page for SoterOne Mainnet](https://thirdweb.com/soterone).
...
5. After modifying your contract with your desired custom logic, you may deploy it to SoterOne Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select SoterOne Mainnet as the network
...
### Initialize SDK On SoterOne Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to SoterOne Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Soterone } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Soterone}>
      <YourApp />
    </ThirdwebProvider>
  );
};
