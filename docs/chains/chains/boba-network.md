...
# Building contracts and apps on Boba Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Boba Network. By the end of the guide, you should be able to create smart contracts and dApps on Boba Network. If you want to check details such as RPCs for Boba Network, check out our [chainlist page for Boba Network](https://thirdweb.com/boba-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Boba Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Boba Network as the network
...
### Initialize SDK On Boba Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Boba Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { BobaNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={BobaNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
