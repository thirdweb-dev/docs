...
5. After modifying your contract with your desired custom logic, you may deploy it to Evanesco Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Evanesco Mainnet as the network
...
### Initialize SDK On Evanesco Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Evanesco Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Evanesco } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Evanesco}>
      <YourApp />
    </ThirdwebProvider>
  );
};
