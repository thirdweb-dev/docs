...
# Building contracts and apps on Genesis Coin

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Genesis Coin. By the end of the guide, you should be able to create smart contracts and dApps on Genesis Coin. If you want to check details such as RPCs for Genesis Coin, check out our [chainlist page for Genesis Coin](https://thirdweb.com/genesis-coin).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Genesis Coin using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Genesis Coin as the network
...
### Initialize SDK On Genesis Coin

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Genesis Coin

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { GenesisCoin } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={GenesisCoin}>
      <YourApp />
    </ThirdwebProvider>
  );
};
