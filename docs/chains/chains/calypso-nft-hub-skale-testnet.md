---
title: Calypso NFT Hub (SKALE Testnet)
slug: /calypso-nft-hub-skale-testnet
hide_title: true
displayed_sidebar: chains
---

# Building contracts and apps on Calypso NFT Hub (SKALE Testnet)

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Calypso NFT Hub (SKALE Testnet). By the end of the guide, you should be able to create smart contracts and dApps on Calypso NFT Hub (SKALE Testnet). If you want to check details such as RPCs for Calypso NFT Hub (SKALE Testnet), check out our [chainlist page for Calypso NFT Hub (SKALE Testnet)](https://thirdweb.com/calypso-nft-hub-skale-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Calypso NFT Hub (SKALE Testnet) using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Calypso NFT Hub (SKALE Testnet) as the network
...
### Initialize SDK On Calypso NFT Hub (SKALE Testnet)

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Calypso NFT Hub (SKALE Testnet)

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { CalypsoNftHubSkaleTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={CalypsoNftHubSkaleTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
