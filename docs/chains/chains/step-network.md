...
# Building contracts and apps on Step Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Step Network. By the end of the guide, you should be able to create smart contracts and dApps on Step Network. If you want to check details such as RPCs for Step Network, check out our [chainlist page for Step Network](https://thirdweb.com/step-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Step Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Step Network as the network
...
### Initialize SDK On Step Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Step Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { StepNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={StepNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
