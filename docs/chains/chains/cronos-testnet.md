...
# Building contracts and apps on Cronos Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Cronos Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Cronos Testnet. If you want to check details such as RPCs for Cronos Testnet, check out our [chainlist page for Cronos Testnet](https://thirdweb.com/cronos-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Cronos Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Cronos Testnet as the network
...
### Initialize SDK On Cronos Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Cronos Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { CronosTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={CronosTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
