...
# Building contracts and apps on Sherpax Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Sherpax Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Sherpax Testnet. If you want to check details such as RPCs for Sherpax Testnet, check out our [chainlist page for Sherpax Testnet](https://thirdweb.com/sherpax-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Sherpax Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Sherpax Testnet as the network
...
### Initialize SDK On Sherpax Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Sherpax Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { SherpaxTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={SherpaxTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
