...
# Building contracts and apps on Bear Network Chain Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Bear Network Chain Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on Bear Network Chain Mainnet. If you want to check details such as RPCs for Bear Network Chain Mainnet, check out our [chainlist page for Bear Network Chain Mainnet](https://thirdweb.com/bear-network-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Bear Network Chain Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Bear Network Chain Mainnet as the network
...
### Initialize SDK On Bear Network Chain Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Bear Network Chain Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { BearNetworkChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={BearNetworkChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
