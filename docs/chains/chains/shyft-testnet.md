...
# Building contracts and apps on Shyft Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Shyft Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Shyft Testnet. If you want to check details such as RPCs for Shyft Testnet, check out our [chainlist page for Shyft Testnet](https://thirdweb.com/shyft-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Shyft Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Shyft Testnet as the network
...
### Initialize SDK On Shyft Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Shyft Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ShyftTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ShyftTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
