...
# Building contracts and apps on Arbitrum Rinkeby

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Arbitrum Rinkeby. By the end of the guide, you should be able to create smart contracts and dApps on Arbitrum Rinkeby. If you want to check details such as RPCs for Arbitrum Rinkeby, check out our [chainlist page for Arbitrum Rinkeby](https://thirdweb.com/arbitrum-rinkeby).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Arbitrum Rinkeby using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Arbitrum Rinkeby as the network
...
### Initialize SDK On Arbitrum Rinkeby

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Arbitrum Rinkeby

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ArbitrumRinkeby } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ArbitrumRinkeby}>
      <YourApp />
    </ThirdwebProvider>
  );
};
