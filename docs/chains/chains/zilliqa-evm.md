...
# Building contracts and apps on Zilliqa EVM

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Zilliqa EVM. By the end of the guide, you should be able to create smart contracts and dApps on Zilliqa EVM. If you want to check details such as RPCs for Zilliqa EVM, check out our [chainlist page for Zilliqa EVM](https://thirdweb.com/zilliqa-evm).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Zilliqa EVM using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Zilliqa EVM as the network
...
### Initialize SDK On Zilliqa EVM

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Zilliqa EVM

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ZilliqaEvm } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ZilliqaEvm}>
      <YourApp />
    </ThirdwebProvider>
  );
};
