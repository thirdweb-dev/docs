...
# Building contracts and apps on AIOZ Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on AIOZ Network. By the end of the guide, you should be able to create smart contracts and dApps on AIOZ Network. If you want to check details such as RPCs for AIOZ Network, check out our [chainlist page for AIOZ Network](https://thirdweb.com/aioz-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to AIOZ Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select AIOZ Network as the network
...
### Initialize SDK On AIOZ Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to AIOZ Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { AiozNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={AiozNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
