...
# Building contracts and apps on QuarkChain Devnet Root

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on QuarkChain Devnet Root. By the end of the guide, you should be able to create smart contracts and dApps on QuarkChain Devnet Root. If you want to check details such as RPCs for QuarkChain Devnet Root, check out our [chainlist page for QuarkChain Devnet Root](https://thirdweb.com/quarkchain-devnet-root).
...
5. After modifying your contract with your desired custom logic, you may deploy it to QuarkChain Devnet Root using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select QuarkChain Devnet Root as the network
...
### Initialize SDK On QuarkChain Devnet Root

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to QuarkChain Devnet Root

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { QuarkchainDevnetRoot } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={QuarkchainDevnetRoot}>
      <YourApp />
    </ThirdwebProvider>
  );
};
