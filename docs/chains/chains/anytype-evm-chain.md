...
# Building contracts and apps on Anytype EVM Chain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Anytype EVM Chain. By the end of the guide, you should be able to create smart contracts and dApps on Anytype EVM Chain. If you want to check details such as RPCs for Anytype EVM Chain, check out our [chainlist page for Anytype EVM Chain](https://thirdweb.com/anytype-evm-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Anytype EVM Chain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Anytype EVM Chain as the network
...
### Initialize SDK On Anytype EVM Chain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Anytype EVM Chain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { AnytypeEvmChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={AnytypeEvmChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
