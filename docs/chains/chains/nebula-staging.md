...
# Building contracts and apps on Nebula Staging

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Nebula Staging. By the end of the guide, you should be able to create smart contracts and dApps on Nebula Staging. If you want to check details such as RPCs for Nebula Staging, check out our [chainlist page for Nebula Staging](https://thirdweb.com/nebula-staging).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Nebula Staging using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Nebula Staging as the network
...
### Initialize SDK On Nebula Staging

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Nebula Staging

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { NebulaStaging } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={NebulaStaging}>
      <YourApp />
    </ThirdwebProvider>
  );
};
