...
# Building contracts and apps on iExec Sidechain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on iExec Sidechain. By the end of the guide, you should be able to create smart contracts and dApps on iExec Sidechain. If you want to check details such as RPCs for iExec Sidechain, check out our [chainlist page for iExec Sidechain](https://thirdweb.com/iexec-sidechain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to iExec Sidechain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select iExec Sidechain as the network
...
### Initialize SDK On iExec Sidechain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to iExec Sidechain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { IexecSidechain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={IexecSidechain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
