...
# Building contracts and apps on PolySmartChain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on PolySmartChain. By the end of the guide, you should be able to create smart contracts and dApps on PolySmartChain. If you want to check details such as RPCs for PolySmartChain, check out our [chainlist page for PolySmartChain](https://thirdweb.com/polysmartchain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to PolySmartChain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select PolySmartChain as the network
...
### Initialize SDK On PolySmartChain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to PolySmartChain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Polysmartchain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Polysmartchain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
