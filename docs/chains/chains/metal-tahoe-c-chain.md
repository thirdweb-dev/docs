...
# Building contracts and apps on Metal Tahoe C-Chain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Metal Tahoe C-Chain. By the end of the guide, you should be able to create smart contracts and dApps on Metal Tahoe C-Chain. If you want to check details such as RPCs for Metal Tahoe C-Chain, check out our [chainlist page for Metal Tahoe C-Chain](https://thirdweb.com/metal-tahoe-c-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Metal Tahoe C-Chain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Metal Tahoe C-Chain as the network
...
### Initialize SDK On Metal Tahoe C-Chain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Metal Tahoe C-Chain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { MetalTahoeCChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={MetalTahoeCChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
