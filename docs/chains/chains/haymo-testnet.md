...
# Building contracts and apps on Haymo Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Haymo Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Haymo Testnet. If you want to check details such as RPCs for Haymo Testnet, check out our [chainlist page for Haymo Testnet](https://thirdweb.com/haymo-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Haymo Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Haymo Testnet as the network
...
### Initialize SDK On Haymo Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Haymo Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { HaymoTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={HaymoTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
