...
# Building contracts and apps on Alaya Dev Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Alaya Dev Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Alaya Dev Testnet. If you want to check details such as RPCs for Alaya Dev Testnet, check out our [chainlist page for Alaya Dev Testnet](https://thirdweb.com/alaya-dev-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Alaya Dev Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Alaya Dev Testnet as the network
...
### Initialize SDK On Alaya Dev Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Alaya Dev Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { AlayaDevTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={AlayaDevTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
