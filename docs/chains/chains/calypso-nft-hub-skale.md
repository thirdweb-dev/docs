...
5. After modifying your contract with your desired custom logic, you may deploy it to Calypso NFT Hub (SKALE) using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Calypso NFT Hub (SKALE) as the network
...
### Initialize SDK On Calypso NFT Hub (SKALE)

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Calypso NFT Hub (SKALE)

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { CalypsoNftHubSkale } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={CalypsoNftHubSkale}>
      <YourApp />
    </ThirdwebProvider>
  );
};
