...
# Building contracts and apps on Toki Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Toki Network. By the end of the guide, you should be able to create smart contracts and dApps on Toki Network. If you want to check details such as RPCs for Toki Network, check out our [chainlist page for Toki Network](https://thirdweb.com/toki-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Toki Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Toki Network as the network
...
### Initialize SDK On Toki Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Toki Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { TokiNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={TokiNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
