...
# Building contracts and apps on Neon EVM DevNet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Neon EVM DevNet. By the end of the guide, you should be able to create smart contracts and dApps on Neon EVM DevNet. If you want to check details such as RPCs for Neon EVM DevNet, check out our [chainlist page for Neon EVM DevNet](https://thirdweb.com/neon-evm-devnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Neon EVM DevNet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Neon EVM DevNet as the network
...
### Initialize SDK On Neon EVM DevNet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Neon EVM DevNet
...
const App = () => {
  return (
    <ThirdwebProvider activeChain={NeonEvmDevnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
