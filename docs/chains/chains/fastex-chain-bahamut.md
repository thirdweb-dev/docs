---
title: Fastex Chain (Bahamut)
slug: /fastex-chain-bahamut
hide_title: true
displayed_sidebar: chains
---

# Building contracts and apps on Fastex Chain (Bahamut)

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Fastex Chain (Bahamut). By the end of the guide, you should be able to create smart contracts and dApps on Fastex Chain (Bahamut). If you want to check details such as RPCs for Fastex Chain (Bahamut), check out our [chainlist page for Fastex Chain (Bahamut)](https://thirdweb.com/fastex-chain-bahamut).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Fastex Chain (Bahamut) using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Fastex Chain (Bahamut) as the network
...
### Initialize SDK On Fastex Chain (Bahamut)

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Fastex Chain (Bahamut)

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { FastexChainBahamut } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={FastexChainBahamut}>
      <YourApp />
    </ThirdwebProvider>
  );
};
