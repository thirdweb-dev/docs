...
# Building contracts and apps on Chaos (SKALE Testnet)

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Chaos (SKALE Testnet). By the end of the guide, you should be able to create smart contracts and dApps on Chaos (SKALE Testnet). If you want to check details such as RPCs for Chaos (SKALE Testnet), check out our [chainlist page for Chaos (SKALE Testnet)](https://thirdweb.com/chaos-skale-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Chaos (SKALE Testnet) using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Chaos (SKALE Testnet) as the network
...
### Initialize SDK On Chaos (SKALE Testnet)

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Chaos (SKALE Testnet)

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ChaosSkaleTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ChaosSkaleTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
