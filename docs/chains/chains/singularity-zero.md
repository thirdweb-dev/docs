...
# Building contracts and apps on Singularity ZERO Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Singularity ZERO Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on Singularity ZERO Mainnet. If you want to check details such as RPCs for Singularity ZERO Mainnet, check out our [chainlist page for Singularity ZERO Mainnet](https://thirdweb.com/singularity-zero).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Singularity ZERO Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Singularity ZERO Mainnet as the network
...
### Initialize SDK On Singularity ZERO Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Singularity ZERO Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { SingularityZero } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={SingularityZero}>
      <YourApp />
    </ThirdwebProvider>
  );
};
