...
5. After modifying your contract with your desired custom logic, you may deploy it to Polis Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Polis Mainnet as the network
...
### Initialize SDK On Polis Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Polis Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Polis } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Polis}>
      <YourApp />
    </ThirdwebProvider>
  );
};
