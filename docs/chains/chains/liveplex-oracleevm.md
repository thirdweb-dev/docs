...
# Building contracts and apps on Liveplex OracleEVM

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Liveplex OracleEVM. By the end of the guide, you should be able to create smart contracts and dApps on Liveplex OracleEVM. If you want to check details such as RPCs for Liveplex OracleEVM, check out our [chainlist page for Liveplex OracleEVM](https://thirdweb.com/liveplex-oracleevm).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Liveplex OracleEVM using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Liveplex OracleEVM as the network
...
### Initialize SDK On Liveplex OracleEVM

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Liveplex OracleEVM

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { LiveplexOracleevm } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={LiveplexOracleevm}>
      <YourApp />
    </ThirdwebProvider>
  );
};
