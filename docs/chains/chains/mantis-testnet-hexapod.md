...
# Building contracts and apps on Mantis Testnet (Hexapod)

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Mantis Testnet (Hexapod). By the end of the guide, you should be able to create smart contracts and dApps on Mantis Testnet (Hexapod). If you want to check details such as RPCs for Mantis Testnet (Hexapod), check out our [chainlist page for Mantis Testnet (Hexapod)](https://thirdweb.com/mantis-testnet-hexapod).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Mantis Testnet (Hexapod) using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Mantis Testnet (Hexapod) as the network
...
### Initialize SDK On Mantis Testnet (Hexapod)

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Mantis Testnet (Hexapod)

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { MantisTestnetHexapod } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={MantisTestnetHexapod}>
      <YourApp />
    </ThirdwebProvider>
  );
};
