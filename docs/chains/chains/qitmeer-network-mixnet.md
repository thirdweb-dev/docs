...
# Building contracts and apps on Qitmeer Network Mixnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Qitmeer Network Mixnet. By the end of the guide, you should be able to create smart contracts and dApps on Qitmeer Network Mixnet. If you want to check details such as RPCs for Qitmeer Network Mixnet, check out our [chainlist page for Qitmeer Network Mixnet](https://thirdweb.com/qitmeer-network-mixnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Qitmeer Network Mixnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Qitmeer Network Mixnet as the network
...
### Initialize SDK On Qitmeer Network Mixnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Qitmeer Network Mixnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { QitmeerNetworkMixnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={QitmeerNetworkMixnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
