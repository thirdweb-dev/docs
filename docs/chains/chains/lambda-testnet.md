...
# Building contracts and apps on Lambda Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Lambda Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Lambda Testnet. If you want to check details such as RPCs for Lambda Testnet, check out our [chainlist page for Lambda Testnet](https://thirdweb.com/lambda-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Lambda Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Lambda Testnet as the network
...
### Initialize SDK On Lambda Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Lambda Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { LambdaTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={LambdaTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
