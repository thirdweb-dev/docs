...
# Building contracts and apps on Kroma Sepolia

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Kroma Sepolia. By the end of the guide, you should be able to create smart contracts and dApps on Kroma Sepolia. If you want to check details such as RPCs for Kroma Sepolia, check out our [chainlist page for Kroma Sepolia](https://thirdweb.com/kroma-sepolia).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Kroma Sepolia using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Kroma Sepolia as the network
...
### Initialize SDK On Kroma Sepolia

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Kroma Sepolia

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { KromaSepolia } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={KromaSepolia}>
      <YourApp />
    </ThirdwebProvider>
  );
};
