...
# Building contracts and apps on Rollux Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Rollux Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Rollux Testnet. If you want to check details such as RPCs for Rollux Testnet, check out our [chainlist page for Rollux Testnet](https://thirdweb.com/rollux-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Rollux Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Rollux Testnet as the network
...
### Initialize SDK On Rollux Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Rollux Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { RolluxTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={RolluxTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
