...
# Building contracts and apps on Plian Mainnet Main

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Plian Mainnet Main. By the end of the guide, you should be able to create smart contracts and dApps on Plian Mainnet Main. If you want to check details such as RPCs for Plian Mainnet Main, check out our [chainlist page for Plian Mainnet Main](https://thirdweb.com/plian-main).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Plian Mainnet Main using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Plian Mainnet Main as the network
...
### Initialize SDK On Plian Mainnet Main

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Plian Mainnet Main

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { PlianMain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={PlianMain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
