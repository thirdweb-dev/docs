...
# Building contracts and apps on AME Chain Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on AME Chain Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on AME Chain Mainnet. If you want to check details such as RPCs for AME Chain Mainnet, check out our [chainlist page for AME Chain Mainnet](https://thirdweb.com/ame-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to AME Chain Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select AME Chain Mainnet as the network
...
### Initialize SDK On AME Chain Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to AME Chain Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { AmeChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={AmeChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
