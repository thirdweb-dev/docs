...
# Building contracts and apps on Htmlcoin Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Htmlcoin Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on Htmlcoin Mainnet. If you want to check details such as RPCs for Htmlcoin Mainnet, check out our [chainlist page for Htmlcoin Mainnet](https://thirdweb.com/htmlcoin).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Htmlcoin Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Htmlcoin Mainnet as the network
...
### Initialize SDK On Htmlcoin Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Htmlcoin Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Htmlcoin } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Htmlcoin}>
      <YourApp />
    </ThirdwebProvider>
  );
};
