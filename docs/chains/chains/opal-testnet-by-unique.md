...
# Building contracts and apps on Opal testnet by Unique

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Opal testnet by Unique. By the end of the guide, you should be able to create smart contracts and dApps on Opal testnet by Unique. If you want to check details such as RPCs for Opal testnet by Unique, check out our [chainlist page for Opal testnet by Unique](https://thirdweb.com/opal-testnet-by-unique).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Opal testnet by Unique using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Opal testnet by Unique as the network
...
### Initialize SDK On Opal testnet by Unique

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Opal testnet by Unique

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { OpalTestnetByUnique } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={OpalTestnetByUnique}>
      <YourApp />
    </ThirdwebProvider>
  );
};
