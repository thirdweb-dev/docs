...
# Building contracts and apps on IPOS Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on IPOS Network. By the end of the guide, you should be able to create smart contracts and dApps on IPOS Network. If you want to check details such as RPCs for IPOS Network, check out our [chainlist page for IPOS Network](https://thirdweb.com/ipos-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to IPOS Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select IPOS Network as the network
...
### Initialize SDK On IPOS Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to IPOS Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { IposNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={IposNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
