...
# Building contracts and apps on Mythical Chain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Mythical Chain. By the end of the guide, you should be able to create smart contracts and dApps on Mythical Chain. If you want to check details such as RPCs for Mythical Chain, check out our [chainlist page for Mythical Chain](https://thirdweb.com/mythical-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Mythical Chain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Mythical Chain as the network
...
### Initialize SDK On Mythical Chain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Mythical Chain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { MythicalChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={MythicalChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
