...
5. After modifying your contract with your desired custom logic, you may deploy it to OKBChain Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select OKBChain Mainnet as the network
...
### Initialize SDK On OKBChain Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to OKBChain Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Okbchain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Okbchain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
