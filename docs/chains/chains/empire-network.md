...
# Building contracts and apps on Empire Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Empire Network. By the end of the guide, you should be able to create smart contracts and dApps on Empire Network. If you want to check details such as RPCs for Empire Network, check out our [chainlist page for Empire Network](https://thirdweb.com/empire-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Empire Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Empire Network as the network
...
### Initialize SDK On Empire Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Empire Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { EmpireNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={EmpireNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
