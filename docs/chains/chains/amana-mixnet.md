...
# Building contracts and apps on Amana Mixnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Amana Mixnet. By the end of the guide, you should be able to create smart contracts and dApps on Amana Mixnet. If you want to check details such as RPCs for Amana Mixnet, check out our [chainlist page for Amana Mixnet](https://thirdweb.com/amana-mixnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Amana Mixnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Amana Mixnet as the network
...
### Initialize SDK On Amana Mixnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Amana Mixnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { AmanaMixnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={AmanaMixnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
