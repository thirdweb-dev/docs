...
# Building contracts and apps on DBChain Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on DBChain Testnet. By the end of the guide, you should be able to create smart contracts and dApps on DBChain Testnet. If you want to check details such as RPCs for DBChain Testnet, check out our [chainlist page for DBChain Testnet](https://thirdweb.com/dbchain-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to DBChain Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select DBChain Testnet as the network
...
### Initialize SDK On DBChain Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to DBChain Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { DbchainTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={DbchainTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
