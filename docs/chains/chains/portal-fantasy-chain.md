...
# Building contracts and apps on Portal Fantasy Chain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Portal Fantasy Chain. By the end of the guide, you should be able to create smart contracts and dApps on Portal Fantasy Chain. If you want to check details such as RPCs for Portal Fantasy Chain, check out our [chainlist page for Portal Fantasy Chain](https://thirdweb.com/portal-fantasy-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Portal Fantasy Chain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Portal Fantasy Chain as the network
...
### Initialize SDK On Portal Fantasy Chain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Portal Fantasy Chain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { PortalFantasyChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={PortalFantasyChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
