...
# Building contracts and apps on Cosmic Chain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Cosmic Chain. By the end of the guide, you should be able to create smart contracts and dApps on Cosmic Chain. If you want to check details such as RPCs for Cosmic Chain, check out our [chainlist page for Cosmic Chain](https://thirdweb.com/cosmic-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Cosmic Chain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Cosmic Chain as the network
...
### Initialize SDK On Cosmic Chain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Cosmic Chain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { CosmicChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={CosmicChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
