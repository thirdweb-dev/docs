...
# Building contracts and apps on Alph Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Alph Network. By the end of the guide, you should be able to create smart contracts and dApps on Alph Network. If you want to check details such as RPCs for Alph Network, check out our [chainlist page for Alph Network](https://thirdweb.com/alph-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Alph Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Alph Network as the network
...
### Initialize SDK On Alph Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Alph Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { AlphNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={AlphNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
