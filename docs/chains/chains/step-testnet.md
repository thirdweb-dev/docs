...
# Building contracts and apps on Step Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Step Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Step Testnet. If you want to check details such as RPCs for Step Testnet, check out our [chainlist page for Step Testnet](https://thirdweb.com/step-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Step Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Step Testnet as the network
...
### Initialize SDK On Step Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Step Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { StepTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={StepTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
