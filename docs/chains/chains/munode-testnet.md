...
# Building contracts and apps on muNode Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on muNode Testnet. By the end of the guide, you should be able to create smart contracts and dApps on muNode Testnet. If you want to check details such as RPCs for muNode Testnet, check out our [chainlist page for muNode Testnet](https://thirdweb.com/munode-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to muNode Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select muNode Testnet as the network
...
### Initialize SDK On muNode Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to muNode Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { MunodeTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={MunodeTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
