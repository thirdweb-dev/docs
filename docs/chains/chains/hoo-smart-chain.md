...
# Building contracts and apps on Hoo Smart Chain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Hoo Smart Chain. By the end of the guide, you should be able to create smart contracts and dApps on Hoo Smart Chain. If you want to check details such as RPCs for Hoo Smart Chain, check out our [chainlist page for Hoo Smart Chain](https://thirdweb.com/hoo-smart-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Hoo Smart Chain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Hoo Smart Chain as the network
...
### Initialize SDK On Hoo Smart Chain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Hoo Smart Chain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { HooSmartChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={HooSmartChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
