...
# Building contracts and apps on Beagle Messaging Chain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Beagle Messaging Chain. By the end of the guide, you should be able to create smart contracts and dApps on Beagle Messaging Chain. If you want to check details such as RPCs for Beagle Messaging Chain, check out our [chainlist page for Beagle Messaging Chain](https://thirdweb.com/beagle-messaging-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Beagle Messaging Chain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Beagle Messaging Chain as the network
...
### Initialize SDK On Beagle Messaging Chain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Beagle Messaging Chain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { BeagleMessagingChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={BeagleMessagingChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
