...
# Building contracts and apps on Flana Mixnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Flana Mixnet. By the end of the guide, you should be able to create smart contracts and dApps on Flana Mixnet. If you want to check details such as RPCs for Flana Mixnet, check out our [chainlist page for Flana Mixnet](https://thirdweb.com/flana-mixnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Flana Mixnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Flana Mixnet as the network
...
### Initialize SDK On Flana Mixnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Flana Mixnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { FlanaMixnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={FlanaMixnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
