...
# Building contracts and apps on Trust EVM Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Trust EVM Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Trust EVM Testnet. If you want to check details such as RPCs for Trust EVM Testnet, check out our [chainlist page for Trust EVM Testnet](https://thirdweb.com/trust-evm-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Trust EVM Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Trust EVM Testnet as the network
...
### Initialize SDK On Trust EVM Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Trust EVM Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { TrustEvmTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={TrustEvmTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
