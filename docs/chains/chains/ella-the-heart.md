...
# Building contracts and apps on Ella the heart

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Ella the heart. By the end of the guide, you should be able to create smart contracts and dApps on Ella the heart. If you want to check details such as RPCs for Ella the heart, check out our [chainlist page for Ella the heart](https://thirdweb.com/ella-the-heart).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Ella the heart using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Ella the heart as the network
...
### Initialize SDK On Ella the heart

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Ella the heart

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { EllaTheHeart } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={EllaTheHeart}>
      <YourApp />
    </ThirdwebProvider>
  );
};
