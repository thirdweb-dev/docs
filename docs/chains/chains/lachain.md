...
# Building contracts and apps on LACHAIN Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on LACHAIN Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on LACHAIN Mainnet. If you want to check details such as RPCs for LACHAIN Mainnet, check out our [chainlist page for LACHAIN Mainnet](https://thirdweb.com/lachain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to LACHAIN Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select LACHAIN Mainnet as the network
...
### Initialize SDK On LACHAIN Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to LACHAIN Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Lachain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Lachain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
