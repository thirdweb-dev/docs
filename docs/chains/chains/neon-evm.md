...
# Building contracts and apps on Neon EVM MainNet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Neon EVM MainNet. By the end of the guide, you should be able to create smart contracts and dApps on Neon EVM MainNet. If you want to check details such as RPCs for Neon EVM MainNet, check out our [chainlist page for Neon EVM MainNet](https://thirdweb.com/neon-evm).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Neon EVM MainNet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Neon EVM MainNet as the network
...
### Initialize SDK On Neon EVM MainNet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Neon EVM MainNet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { NeonEvm } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={NeonEvm}>
      <YourApp />
    </ThirdwebProvider>
  );
};
