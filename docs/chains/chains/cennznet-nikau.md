...
# Building contracts and apps on CENNZnet Nikau

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on CENNZnet Nikau. By the end of the guide, you should be able to create smart contracts and dApps on CENNZnet Nikau. If you want to check details such as RPCs for CENNZnet Nikau, check out our [chainlist page for CENNZnet Nikau](https://thirdweb.com/cennznet-nikau).
...
5. After modifying your contract with your desired custom logic, you may deploy it to CENNZnet Nikau using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select CENNZnet Nikau as the network
...
### Initialize SDK On CENNZnet Nikau

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to CENNZnet Nikau

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { CennznetNikau } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={CennznetNikau}>
      <YourApp />
    </ThirdwebProvider>
  );
};
