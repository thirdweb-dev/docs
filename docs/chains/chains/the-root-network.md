...
5. After modifying your contract with your desired custom logic, you may deploy it to The Root Network - Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select The Root Network - Mainnet as the network
...
### Initialize SDK On The Root Network - Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to The Root Network - Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { TheRootNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={TheRootNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
