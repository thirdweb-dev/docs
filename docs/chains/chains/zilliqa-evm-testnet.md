...
# Building contracts and apps on Zilliqa EVM Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Zilliqa EVM Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Zilliqa EVM Testnet. If you want to check details such as RPCs for Zilliqa EVM Testnet, check out our [chainlist page for Zilliqa EVM Testnet](https://thirdweb.com/zilliqa-evm-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Zilliqa EVM Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Zilliqa EVM Testnet as the network
...
### Initialize SDK On Zilliqa EVM Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Zilliqa EVM Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ZilliqaEvmTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ZilliqaEvmTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
