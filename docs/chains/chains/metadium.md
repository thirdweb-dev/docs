...
# Building contracts and apps on Metadium Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Metadium Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on Metadium Mainnet. If you want to check details such as RPCs for Metadium Mainnet, check out our [chainlist page for Metadium Mainnet](https://thirdweb.com/metadium).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Metadium Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Metadium Mainnet as the network
...
### Initialize SDK On Metadium Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Metadium Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Metadium } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Metadium}>
      <YourApp />
    </ThirdwebProvider>
  );
};
