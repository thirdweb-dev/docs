...
# Building contracts and apps on Hazlor Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Hazlor Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Hazlor Testnet. If you want to check details such as RPCs for Hazlor Testnet, check out our [chainlist page for Hazlor Testnet](https://thirdweb.com/hazlor-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Hazlor Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Hazlor Testnet as the network
...
### Initialize SDK On Hazlor Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Hazlor Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { HazlorTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={HazlorTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
