...
# Building contracts and apps on DRAC Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on DRAC Network. By the end of the guide, you should be able to create smart contracts and dApps on DRAC Network. If you want to check details such as RPCs for DRAC Network, check out our [chainlist page for DRAC Network](https://thirdweb.com/drac-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to DRAC Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select DRAC Network as the network
...
### Initialize SDK On DRAC Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to DRAC Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { DracNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={DracNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
