...
# Building contracts and apps on Metacodechain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Metacodechain. By the end of the guide, you should be able to create smart contracts and dApps on Metacodechain. If you want to check details such as RPCs for Metacodechain, check out our [chainlist page for Metacodechain](https://thirdweb.com/metacodechain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Metacodechain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Metacodechain as the network
...
### Initialize SDK On Metacodechain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Metacodechain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Metacodechain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Metacodechain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
