...
# Building contracts and apps on Evrice Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Evrice Network. By the end of the guide, you should be able to create smart contracts and dApps on Evrice Network. If you want to check details such as RPCs for Evrice Network, check out our [chainlist page for Evrice Network](https://thirdweb.com/evrice-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Evrice Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Evrice Network as the network
...
### Initialize SDK On Evrice Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Evrice Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { EvriceNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={EvriceNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
