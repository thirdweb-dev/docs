...
# Building contracts and apps on TBWG Chain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on TBWG Chain. By the end of the guide, you should be able to create smart contracts and dApps on TBWG Chain. If you want to check details such as RPCs for TBWG Chain, check out our [chainlist page for TBWG Chain](https://thirdweb.com/tbwg-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to TBWG Chain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select TBWG Chain as the network
...
### Initialize SDK On TBWG Chain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to TBWG Chain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { TbwgChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={TbwgChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
