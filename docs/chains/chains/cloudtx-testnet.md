...
# Building contracts and apps on CloudTx Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on CloudTx Testnet. By the end of the guide, you should be able to create smart contracts and dApps on CloudTx Testnet. If you want to check details such as RPCs for CloudTx Testnet, check out our [chainlist page for CloudTx Testnet](https://thirdweb.com/cloudtx-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to CloudTx Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select CloudTx Testnet as the network
...
### Initialize SDK On CloudTx Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to CloudTx Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { CloudtxTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={CloudtxTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
