---
title: Taiko (Alpha-3 Testnet)
slug: /taiko-alpha-3-testnet
hide_title: true
displayed_sidebar: chains
---

# Building contracts and apps on Taiko (Alpha-3 Testnet)

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Taiko (Alpha-3 Testnet). By the end of the guide, you should be able to create smart contracts and dApps on Taiko (Alpha-3 Testnet). If you want to check details such as RPCs for Taiko (Alpha-3 Testnet), check out our [chainlist page for Taiko (Alpha-3 Testnet)](https://thirdweb.com/taiko-alpha-3-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Taiko (Alpha-3 Testnet) using [Deploy](https://portal.thirdweb.com/deploy).
...
### Initialize SDK On Taiko (Alpha-3 Testnet)

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Taiko (Alpha-3 Testnet)

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { TaikoAlpha3Testnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={TaikoAlpha3Testnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
