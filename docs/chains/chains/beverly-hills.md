...
# Building contracts and apps on Beverly Hills

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Beverly Hills. By the end of the guide, you should be able to create smart contracts and dApps on Beverly Hills. If you want to check details such as RPCs for Beverly Hills, check out our [chainlist page for Beverly Hills](https://thirdweb.com/beverly-hills).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Beverly Hills using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Beverly Hills as the network
...
### Initialize SDK On Beverly Hills

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Beverly Hills

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { BeverlyHills } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={BeverlyHills}>
      <YourApp />
    </ThirdwebProvider>
  );
};
