...
# Building contracts and apps on AltLayer Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on AltLayer Testnet. By the end of the guide, you should be able to create smart contracts and dApps on AltLayer Testnet. If you want to check details such as RPCs for AltLayer Testnet, check out our [chainlist page for AltLayer Testnet](https://thirdweb.com/altlayer-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to AltLayer Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select AltLayer Testnet as the network
...
### Initialize SDK On AltLayer Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to AltLayer Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { AltlayerTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={AltlayerTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
