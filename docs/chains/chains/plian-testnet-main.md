...
# Building contracts and apps on Plian Testnet Main

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Plian Testnet Main. By the end of the guide, you should be able to create smart contracts and dApps on Plian Testnet Main. If you want to check details such as RPCs for Plian Testnet Main, check out our [chainlist page for Plian Testnet Main](https://thirdweb.com/plian-testnet-main).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Plian Testnet Main using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Plian Testnet Main as the network
...
### Initialize SDK On Plian Testnet Main

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Plian Testnet Main

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { PlianTestnetMain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={PlianTestnetMain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
