...
# Building contracts and apps on Venidium Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Venidium Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on Venidium Mainnet. If you want to check details such as RPCs for Venidium Mainnet, check out our [chainlist page for Venidium Mainnet](https://thirdweb.com/venidium).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Venidium Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Venidium Mainnet as the network
...
### Initialize SDK On Venidium Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Venidium Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Venidium } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Venidium}>
      <YourApp />
    </ThirdwebProvider>
  );
};
