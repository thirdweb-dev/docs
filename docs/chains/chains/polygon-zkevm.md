...
# Building contracts and apps on Polygon zkEVM

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Polygon zkEVM. By the end of the guide, you should be able to create smart contracts and dApps on Polygon zkEVM. If you want to check details such as RPCs for Polygon zkEVM, check out our [chainlist page for Polygon zkEVM](https://thirdweb.com/polygon-zkevm).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Polygon zkEVM using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Polygon zkEVM as the network
...
### Initialize SDK On Polygon zkEVM

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Polygon zkEVM

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { PolygonZkevm } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={PolygonZkevm}>
      <YourApp />
    </ThirdwebProvider>
  );
};
