...
# Building contracts and apps on Diode Testnet Staging

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Diode Testnet Staging. By the end of the guide, you should be able to create smart contracts and dApps on Diode Testnet Staging. If you want to check details such as RPCs for Diode Testnet Staging, check out our [chainlist page for Diode Testnet Staging](https://thirdweb.com/diode-testnet-staging).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Diode Testnet Staging using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Diode Testnet Staging as the network
...
### Initialize SDK On Diode Testnet Staging

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Diode Testnet Staging

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { DiodeTestnetStaging } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={DiodeTestnetStaging}>
      <YourApp />
    </ThirdwebProvider>
  );
};
