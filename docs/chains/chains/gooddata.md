...
# Building contracts and apps on GoodData Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on GoodData Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on GoodData Mainnet. If you want to check details such as RPCs for GoodData Mainnet, check out our [chainlist page for GoodData Mainnet](https://thirdweb.com/gooddata).
...
5. After modifying your contract with your desired custom logic, you may deploy it to GoodData Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select GoodData Mainnet as the network
...
### Initialize SDK On GoodData Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to GoodData Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Gooddata } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Gooddata}>
      <YourApp />
    </ThirdwebProvider>
  );
};
