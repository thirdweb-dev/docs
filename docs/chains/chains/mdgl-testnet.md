...
# Building contracts and apps on MDGL Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on MDGL Testnet. By the end of the guide, you should be able to create smart contracts and dApps on MDGL Testnet. If you want to check details such as RPCs for MDGL Testnet, check out our [chainlist page for MDGL Testnet](https://thirdweb.com/mdgl-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to MDGL Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select MDGL Testnet as the network
...
### Initialize SDK On MDGL Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to MDGL Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { MdglTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={MdglTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
