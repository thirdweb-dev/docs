...
# Building contracts and apps on Mizana Mixnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Mizana Mixnet. By the end of the guide, you should be able to create smart contracts and dApps on Mizana Mixnet. If you want to check details such as RPCs for Mizana Mixnet, check out our [chainlist page for Mizana Mixnet](https://thirdweb.com/mizana-mixnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Mizana Mixnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Mizana Mixnet as the network
...
### Initialize SDK On Mizana Mixnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Mizana Mixnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { MizanaMixnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={MizanaMixnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
