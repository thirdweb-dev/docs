...
# Building contracts and apps on ethereum Fair

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on ethereum Fair. By the end of the guide, you should be able to create smart contracts and dApps on ethereum Fair. If you want to check details such as RPCs for ethereum Fair, check out our [chainlist page for ethereum Fair](https://thirdweb.com/ethereum-fair).
...
5. After modifying your contract with your desired custom logic, you may deploy it to ethereum Fair using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select ethereum Fair as the network
...
### Initialize SDK On ethereum Fair

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to ethereum Fair

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { EthereumFair } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={EthereumFair}>
      <YourApp />
    </ThirdwebProvider>
  );
};
