...
# Building contracts and apps on CryptoCoinPay

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on CryptoCoinPay. By the end of the guide, you should be able to create smart contracts and dApps on CryptoCoinPay. If you want to check details such as RPCs for CryptoCoinPay, check out our [chainlist page for CryptoCoinPay](https://thirdweb.com/cryptocoinpay).
...
5. After modifying your contract with your desired custom logic, you may deploy it to CryptoCoinPay using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select CryptoCoinPay as the network
...
### Initialize SDK On CryptoCoinPay

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to CryptoCoinPay

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Cryptocoinpay } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Cryptocoinpay}>
      <YourApp />
    </ThirdwebProvider>
  );
};
