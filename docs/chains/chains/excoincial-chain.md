...
5. After modifying your contract with your desired custom logic, you may deploy it to Excoincial Chain Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Excoincial Chain Mainnet as the network
...
### Initialize SDK On Excoincial Chain Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Excoincial Chain Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ExcoincialChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ExcoincialChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
