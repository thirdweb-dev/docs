...
# Building contracts and apps on AirDAO Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on AirDAO Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on AirDAO Mainnet. If you want to check details such as RPCs for AirDAO Mainnet, check out our [chainlist page for AirDAO Mainnet](https://thirdweb.com/airdao).
...
5. After modifying your contract with your desired custom logic, you may deploy it to AirDAO Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select AirDAO Mainnet as the network
...
### Initialize SDK On AirDAO Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to AirDAO Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Airdao } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Airdao}>
      <YourApp />
    </ThirdwebProvider>
  );
};
