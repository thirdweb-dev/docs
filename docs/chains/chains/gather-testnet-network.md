...
# Building contracts and apps on Gather Testnet Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Gather Testnet Network. By the end of the guide, you should be able to create smart contracts and dApps on Gather Testnet Network. If you want to check details such as RPCs for Gather Testnet Network, check out our [chainlist page for Gather Testnet Network](https://thirdweb.com/gather-testnet-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Gather Testnet Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Gather Testnet Network as the network
...
### Initialize SDK On Gather Testnet Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Gather Testnet Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { GatherTestnetNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={GatherTestnetNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
