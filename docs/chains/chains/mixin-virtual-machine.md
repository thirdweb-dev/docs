...
# Building contracts and apps on Mixin Virtual Machine

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Mixin Virtual Machine. By the end of the guide, you should be able to create smart contracts and dApps on Mixin Virtual Machine. If you want to check details such as RPCs for Mixin Virtual Machine, check out our [chainlist page for Mixin Virtual Machine](https://thirdweb.com/mixin-virtual-machine).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Mixin Virtual Machine using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Mixin Virtual Machine as the network
...
### Initialize SDK On Mixin Virtual Machine

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Mixin Virtual Machine

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { MixinVirtualMachine } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={MixinVirtualMachine}>
      <YourApp />
    </ThirdwebProvider>
  );
};
