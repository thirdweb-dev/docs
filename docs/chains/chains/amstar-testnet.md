...
# Building contracts and apps on AmStar Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on AmStar Testnet. By the end of the guide, you should be able to create smart contracts and dApps on AmStar Testnet. If you want to check details such as RPCs for AmStar Testnet, check out our [chainlist page for AmStar Testnet](https://thirdweb.com/amstar-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to AmStar Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select AmStar Testnet as the network
...
### Initialize SDK On AmStar Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to AmStar Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { AmstarTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={AmstarTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
