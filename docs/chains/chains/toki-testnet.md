...
# Building contracts and apps on Toki Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Toki Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Toki Testnet. If you want to check details such as RPCs for Toki Testnet, check out our [chainlist page for Toki Testnet](https://thirdweb.com/toki-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Toki Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Toki Testnet as the network
...
### Initialize SDK On Toki Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Toki Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { TokiTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={TokiTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
