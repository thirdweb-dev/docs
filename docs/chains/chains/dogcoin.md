...
5. After modifying your contract with your desired custom logic, you may deploy it to Dogcoin Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Dogcoin Mainnet as the network
...
### Initialize SDK On Dogcoin Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Dogcoin Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Dogcoin } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Dogcoin}>
      <YourApp />
    </ThirdwebProvider>
  );
};
