...
# Building contracts and apps on Darwinia Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Darwinia Network. By the end of the guide, you should be able to create smart contracts and dApps on Darwinia Network. If you want to check details such as RPCs for Darwinia Network, check out our [chainlist page for Darwinia Network](https://thirdweb.com/darwinia-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Darwinia Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Darwinia Network as the network
...
### Initialize SDK On Darwinia Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Darwinia Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { DarwiniaNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={DarwiniaNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
