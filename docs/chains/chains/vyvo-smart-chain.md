...
# Building contracts and apps on Vyvo Smart Chain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Vyvo Smart Chain. By the end of the guide, you should be able to create smart contracts and dApps on Vyvo Smart Chain. If you want to check details such as RPCs for Vyvo Smart Chain, check out our [chainlist page for Vyvo Smart Chain](https://thirdweb.com/vyvo-smart-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Vyvo Smart Chain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Vyvo Smart Chain as the network
...
### Initialize SDK On Vyvo Smart Chain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Vyvo Smart Chain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { VyvoSmartChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={VyvoSmartChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
