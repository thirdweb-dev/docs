...
# Building contracts and apps on Fox Testnet Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Fox Testnet Network. By the end of the guide, you should be able to create smart contracts and dApps on Fox Testnet Network. If you want to check details such as RPCs for Fox Testnet Network, check out our [chainlist page for Fox Testnet Network](https://thirdweb.com/fox-testnet-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Fox Testnet Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Fox Testnet Network as the network
...
### Initialize SDK On Fox Testnet Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Fox Testnet Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { FoxTestnetNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={FoxTestnetNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
