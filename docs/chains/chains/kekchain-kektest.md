...
# Building contracts and apps on Kekchain (kektest)

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Kekchain (kektest). By the end of the guide, you should be able to create smart contracts and dApps on Kekchain (kektest). If you want to check details such as RPCs for Kekchain (kektest), check out our [chainlist page for Kekchain (kektest)](https://thirdweb.com/kekchain-kektest).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Kekchain (kektest) using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Kekchain (kektest) as the network
...
### Initialize SDK On Kekchain (kektest)

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Kekchain (kektest)

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { KekchainKektest } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={KekchainKektest}>
      <YourApp />
    </ThirdwebProvider>
  );
};
