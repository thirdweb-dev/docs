...
# Building contracts and apps on Gather Devnet Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Gather Devnet Network. By the end of the guide, you should be able to create smart contracts and dApps on Gather Devnet Network. If you want to check details such as RPCs for Gather Devnet Network, check out our [chainlist page for Gather Devnet Network](https://thirdweb.com/gather-devnet-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Gather Devnet Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Gather Devnet Network as the network
...
### Initialize SDK On Gather Devnet Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Gather Devnet Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { GatherDevnetNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={GatherDevnetNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
