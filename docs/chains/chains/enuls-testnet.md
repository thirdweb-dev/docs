...
# Building contracts and apps on ENULS Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on ENULS Testnet. By the end of the guide, you should be able to create smart contracts and dApps on ENULS Testnet. If you want to check details such as RPCs for ENULS Testnet, check out our [chainlist page for ENULS Testnet](https://thirdweb.com/enuls-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to ENULS Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select ENULS Testnet as the network
...
### Initialize SDK On ENULS Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to ENULS Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { EnulsTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={EnulsTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
