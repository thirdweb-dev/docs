...
# Building contracts and apps on Aurora Betanet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Aurora Betanet. By the end of the guide, you should be able to create smart contracts and dApps on Aurora Betanet. If you want to check details such as RPCs for Aurora Betanet, check out our [chainlist page for Aurora Betanet](https://thirdweb.com/aurora-betanet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Aurora Betanet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Aurora Betanet as the network
...
### Initialize SDK On Aurora Betanet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Aurora Betanet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { AuroraBetanet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={AuroraBetanet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
