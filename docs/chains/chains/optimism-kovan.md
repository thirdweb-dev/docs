...
# Building contracts and apps on Optimism Kovan

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Optimism Kovan. By the end of the guide, you should be able to create smart contracts and dApps on Optimism Kovan. If you want to check details such as RPCs for Optimism Kovan, check out our [chainlist page for Optimism Kovan](https://thirdweb.com/optimism-kovan).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Optimism Kovan using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Optimism Kovan as the network
...
### Initialize SDK On Optimism Kovan

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Optimism Kovan

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { OptimismKovan } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={OptimismKovan}>
      <YourApp />
    </ThirdwebProvider>
  );
};
