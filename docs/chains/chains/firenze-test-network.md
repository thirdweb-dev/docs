...
# Building contracts and apps on Firenze test network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Firenze test network. By the end of the guide, you should be able to create smart contracts and dApps on Firenze test network. If you want to check details such as RPCs for Firenze test network, check out our [chainlist page for Firenze test network](https://thirdweb.com/firenze-test-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Firenze test network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Firenze test network as the network
...
### Initialize SDK On Firenze test network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Firenze test network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { FirenzeTestNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={FirenzeTestNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
