...
# Building contracts and apps on Etho Protocol

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Etho Protocol. By the end of the guide, you should be able to create smart contracts and dApps on Etho Protocol. If you want to check details such as RPCs for Etho Protocol, check out our [chainlist page for Etho Protocol](https://thirdweb.com/etho-protocol).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Etho Protocol using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Etho Protocol as the network
...
### Initialize SDK On Etho Protocol

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Etho Protocol

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { EthoProtocol } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={EthoProtocol}>
      <YourApp />
    </ThirdwebProvider>
  );
};
