...
# Building contracts and apps on CIC Chain Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on CIC Chain Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on CIC Chain Mainnet. If you want to check details such as RPCs for CIC Chain Mainnet, check out our [chainlist page for CIC Chain Mainnet](https://thirdweb.com/cic-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to CIC Chain Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select CIC Chain Mainnet as the network
...
### Initialize SDK On CIC Chain Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to CIC Chain Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { CicChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={CicChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
