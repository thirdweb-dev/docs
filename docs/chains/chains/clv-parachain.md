...
# Building contracts and apps on CLV Parachain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on CLV Parachain. By the end of the guide, you should be able to create smart contracts and dApps on CLV Parachain. If you want to check details such as RPCs for CLV Parachain, check out our [chainlist page for CLV Parachain](https://thirdweb.com/clv-parachain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to CLV Parachain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select CLV Parachain as the network
...
### Initialize SDK On CLV Parachain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to CLV Parachain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ClvParachain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ClvParachain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
