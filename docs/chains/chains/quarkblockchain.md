...
# Building contracts and apps on quarkblockchain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on quarkblockchain. By the end of the guide, you should be able to create smart contracts and dApps on quarkblockchain. If you want to check details such as RPCs for quarkblockchain, check out our [chainlist page for quarkblockchain](https://thirdweb.com/quarkblockchain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to quarkblockchain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select quarkblockchain as the network
...
### Initialize SDK On quarkblockchain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to quarkblockchain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Quarkblockchain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Quarkblockchain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
