...
# Building contracts and apps on GTON Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on GTON Testnet. By the end of the guide, you should be able to create smart contracts and dApps on GTON Testnet. If you want to check details such as RPCs for GTON Testnet, check out our [chainlist page for GTON Testnet](https://thirdweb.com/gton-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to GTON Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select GTON Testnet as the network
...
### Initialize SDK On GTON Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to GTON Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { GtonTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={GtonTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
