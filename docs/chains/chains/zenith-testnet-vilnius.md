---
title: Zenith Testnet (Vilnius)
slug: /zenith-testnet-vilnius
hide_title: true
displayed_sidebar: chains
---

# Building contracts and apps on Zenith Testnet (Vilnius)

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Zenith Testnet (Vilnius). By the end of the guide, you should be able to create smart contracts and dApps on Zenith Testnet (Vilnius). If you want to check details such as RPCs for Zenith Testnet (Vilnius), check out our [chainlist page for Zenith Testnet (Vilnius)](https://thirdweb.com/zenith-testnet-vilnius).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Zenith Testnet (Vilnius) using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Zenith Testnet (Vilnius) as the network
...
### Initialize SDK On Zenith Testnet (Vilnius)

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Zenith Testnet (Vilnius)

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ZenithTestnetVilnius } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ZenithTestnetVilnius}>
      <YourApp />
    </ThirdwebProvider>
  );
};
