...
# Building contracts and apps on Haradev Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Haradev Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Haradev Testnet. If you want to check details such as RPCs for Haradev Testnet, check out our [chainlist page for Haradev Testnet](https://thirdweb.com/haradev-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Haradev Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Haradev Testnet as the network
...
### Initialize SDK On Haradev Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Haradev Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { HaradevTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={HaradevTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
