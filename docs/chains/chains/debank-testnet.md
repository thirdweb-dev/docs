...
# Building contracts and apps on DeBank Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on DeBank Testnet. By the end of the guide, you should be able to create smart contracts and dApps on DeBank Testnet. If you want to check details such as RPCs for DeBank Testnet, check out our [chainlist page for DeBank Testnet](https://thirdweb.com/debank-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to DeBank Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select DeBank Testnet as the network
...
### Initialize SDK On DeBank Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to DeBank Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { DebankTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={DebankTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
