...
# Building contracts and apps on Theta Amber Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Theta Amber Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Theta Amber Testnet. If you want to check details such as RPCs for Theta Amber Testnet, check out our [chainlist page for Theta Amber Testnet](https://thirdweb.com/theta-amber-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Theta Amber Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Theta Amber Testnet as the network
...
### Initialize SDK On Theta Amber Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Theta Amber Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ThetaAmberTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ThetaAmberTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
