...
# Building contracts and apps on Fastex Chain testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Fastex Chain testnet. By the end of the guide, you should be able to create smart contracts and dApps on Fastex Chain testnet. If you want to check details such as RPCs for Fastex Chain testnet, check out our [chainlist page for Fastex Chain testnet](https://thirdweb.com/fastex-chain-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Fastex Chain testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Fastex Chain testnet as the network
...
### Initialize SDK On Fastex Chain testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Fastex Chain testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { FastexChainTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={FastexChainTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
