...
# Building contracts and apps on Origin Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Origin Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Origin Testnet. If you want to check details such as RPCs for Origin Testnet, check out our [chainlist page for Origin Testnet](https://thirdweb.com/origin-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Origin Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Origin Testnet as the network
...
### Initialize SDK On Origin Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Origin Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { OriginTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={OriginTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
