...
# Building contracts and apps on Uzmi Network Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Uzmi Network Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on Uzmi Network Mainnet. If you want to check details such as RPCs for Uzmi Network Mainnet, check out our [chainlist page for Uzmi Network Mainnet](https://thirdweb.com/uzmi-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Uzmi Network Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Uzmi Network Mainnet as the network
...
### Initialize SDK On Uzmi Network Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Uzmi Network Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { UzmiNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={UzmiNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
