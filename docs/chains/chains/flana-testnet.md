...
# Building contracts and apps on Flana Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Flana Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Flana Testnet. If you want to check details such as RPCs for Flana Testnet, check out our [chainlist page for Flana Testnet](https://thirdweb.com/flana-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Flana Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Flana Testnet as the network
...
### Initialize SDK On Flana Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Flana Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { FlanaTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={FlanaTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
