...
# Building contracts and apps on ONUS Chain Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on ONUS Chain Testnet. By the end of the guide, you should be able to create smart contracts and dApps on ONUS Chain Testnet. If you want to check details such as RPCs for ONUS Chain Testnet, check out our [chainlist page for ONUS Chain Testnet](https://thirdweb.com/onus-chain-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to ONUS Chain Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select ONUS Chain Testnet as the network
...
### Initialize SDK On ONUS Chain Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to ONUS Chain Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { OnusChainTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={OnusChainTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
