...
# Building contracts and apps on TBSI Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on TBSI Testnet. By the end of the guide, you should be able to create smart contracts and dApps on TBSI Testnet. If you want to check details such as RPCs for TBSI Testnet, check out our [chainlist page for TBSI Testnet](https://thirdweb.com/tbsi-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to TBSI Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select TBSI Testnet as the network
...
### Initialize SDK On TBSI Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to TBSI Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { TbsiTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={TbsiTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
