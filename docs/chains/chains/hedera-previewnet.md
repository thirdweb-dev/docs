...
# Building contracts and apps on Hedera Previewnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Hedera Previewnet. By the end of the guide, you should be able to create smart contracts and dApps on Hedera Previewnet. If you want to check details such as RPCs for Hedera Previewnet, check out our [chainlist page for Hedera Previewnet](https://thirdweb.com/hedera-previewnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Hedera Previewnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Hedera Previewnet as the network
...
### Initialize SDK On Hedera Previewnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Hedera Previewnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { HederaPreviewnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={HederaPreviewnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
