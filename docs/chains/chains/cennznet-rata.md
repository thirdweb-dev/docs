...
# Building contracts and apps on CENNZnet Rata

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on CENNZnet Rata. By the end of the guide, you should be able to create smart contracts and dApps on CENNZnet Rata. If you want to check details such as RPCs for CENNZnet Rata, check out our [chainlist page for CENNZnet Rata](https://thirdweb.com/cennznet-rata).
...
5. After modifying your contract with your desired custom logic, you may deploy it to CENNZnet Rata using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select CENNZnet Rata as the network
...
### Initialize SDK On CENNZnet Rata

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to CENNZnet Rata

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { CennznetRata } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={CennznetRata}>
      <YourApp />
    </ThirdwebProvider>
  );
};
