...
# Building contracts and apps on maistestsubnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on maistestsubnet. By the end of the guide, you should be able to create smart contracts and dApps on maistestsubnet. If you want to check details such as RPCs for maistestsubnet, check out our [chainlist page for maistestsubnet](https://thirdweb.com/maistestsubnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to maistestsubnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select maistestsubnet as the network
...
### Initialize SDK On maistestsubnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to maistestsubnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Maistestsubnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Maistestsubnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
