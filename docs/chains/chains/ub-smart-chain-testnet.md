...
# Building contracts and apps on UB Smart Chain(testnet)

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on UB Smart Chain(testnet). By the end of the guide, you should be able to create smart contracts and dApps on UB Smart Chain(testnet). If you want to check details such as RPCs for UB Smart Chain(testnet), check out our [chainlist page for UB Smart Chain(testnet)](https://thirdweb.com/ub-smart-chain-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to UB Smart Chain(testnet) using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select UB Smart Chain(testnet) as the network
...
### Initialize SDK On UB Smart Chain(testnet)

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to UB Smart Chain(testnet)

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { UbSmartChainTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={UbSmartChainTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
