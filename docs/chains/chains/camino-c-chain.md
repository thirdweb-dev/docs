...
# Building contracts and apps on Camino C-Chain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Camino C-Chain. By the end of the guide, you should be able to create smart contracts and dApps on Camino C-Chain. If you want to check details such as RPCs for Camino C-Chain, check out our [chainlist page for Camino C-Chain](https://thirdweb.com/camino-c-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Camino C-Chain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Camino C-Chain as the network
...
### Initialize SDK On Camino C-Chain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Camino C-Chain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { CaminoCChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={CaminoCChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
