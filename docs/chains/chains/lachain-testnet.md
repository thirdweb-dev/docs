...
# Building contracts and apps on LACHAIN Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on LACHAIN Testnet. By the end of the guide, you should be able to create smart contracts and dApps on LACHAIN Testnet. If you want to check details such as RPCs for LACHAIN Testnet, check out our [chainlist page for LACHAIN Testnet](https://thirdweb.com/lachain-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to LACHAIN Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select LACHAIN Testnet as the network
...
### Initialize SDK On LACHAIN Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to LACHAIN Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { LachainTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={LachainTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
