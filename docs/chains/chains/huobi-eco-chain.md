...
5. After modifying your contract with your desired custom logic, you may deploy it to Huobi ECO Chain Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Huobi ECO Chain Mainnet as the network
...
### Initialize SDK On Huobi ECO Chain Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Huobi ECO Chain Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { HuobiEcoChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={HuobiEcoChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
