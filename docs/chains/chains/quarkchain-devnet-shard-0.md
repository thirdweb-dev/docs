...
# Building contracts and apps on QuarkChain Devnet Shard 0

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on QuarkChain Devnet Shard 0. By the end of the guide, you should be able to create smart contracts and dApps on QuarkChain Devnet Shard 0. If you want to check details such as RPCs for QuarkChain Devnet Shard 0, check out our [chainlist page for QuarkChain Devnet Shard 0](https://thirdweb.com/quarkchain-devnet-shard-0).
...
5. After modifying your contract with your desired custom logic, you may deploy it to QuarkChain Devnet Shard 0 using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select QuarkChain Devnet Shard 0 as the network
...
### Initialize SDK On QuarkChain Devnet Shard 0

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to QuarkChain Devnet Shard 0

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { QuarkchainDevnetShard0 } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={QuarkchainDevnetShard0}>
      <YourApp />
    </ThirdwebProvider>
  );
};
