...
# Building contracts and apps on Amana Privnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Amana Privnet. By the end of the guide, you should be able to create smart contracts and dApps on Amana Privnet. If you want to check details such as RPCs for Amana Privnet, check out our [chainlist page for Amana Privnet](https://thirdweb.com/amana-privnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Amana Privnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Amana Privnet as the network
...
### Initialize SDK On Amana Privnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Amana Privnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { AmanaPrivnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={AmanaPrivnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
