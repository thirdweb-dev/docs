...
# Building contracts and apps on Flana Privnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Flana Privnet. By the end of the guide, you should be able to create smart contracts and dApps on Flana Privnet. If you want to check details such as RPCs for Flana Privnet, check out our [chainlist page for Flana Privnet](https://thirdweb.com/flana-privnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Flana Privnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Flana Privnet as the network
...
### Initialize SDK On Flana Privnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Flana Privnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { FlanaPrivnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={FlanaPrivnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
