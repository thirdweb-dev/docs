...
# Building contracts and apps on EOS EVM Network Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on EOS EVM Network Testnet. By the end of the guide, you should be able to create smart contracts and dApps on EOS EVM Network Testnet. If you want to check details such as RPCs for EOS EVM Network Testnet, check out our [chainlist page for EOS EVM Network Testnet](https://thirdweb.com/eos-evm-network-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to EOS EVM Network Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select EOS EVM Network Testnet as the network
...
### Initialize SDK On EOS EVM Network Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to EOS EVM Network Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { EosEvmNetworkTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={EosEvmNetworkTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
