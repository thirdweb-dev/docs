...
# Building contracts and apps on GateChain Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on GateChain Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on GateChain Mainnet. If you want to check details such as RPCs for GateChain Mainnet, check out our [chainlist page for GateChain Mainnet](https://thirdweb.com/gatechain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to GateChain Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select GateChain Mainnet as the network
...
### Initialize SDK On GateChain Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to GateChain Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Gatechain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Gatechain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
