...
# Building contracts and apps on Energy Web Chain

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Energy Web Chain. By the end of the guide, you should be able to create smart contracts and dApps on Energy Web Chain. If you want to check details such as RPCs for Energy Web Chain, check out our [chainlist page for Energy Web Chain](https://thirdweb.com/energy-web-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Energy Web Chain using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Energy Web Chain as the network
...
### Initialize SDK On Energy Web Chain

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Energy Web Chain

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { EnergyWebChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={EnergyWebChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
