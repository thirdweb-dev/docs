...
# Building contracts and apps on TLChain Network Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on TLChain Network Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on TLChain Network Mainnet. If you want to check details such as RPCs for TLChain Network Mainnet, check out our [chainlist page for TLChain Network Mainnet](https://thirdweb.com/tlchain-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to TLChain Network Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select TLChain Network Mainnet as the network
...
### Initialize SDK On TLChain Network Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to TLChain Network Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { TlchainNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={TlchainNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
