...
# Building contracts and apps on Arbitrum on xDai

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Arbitrum on xDai. By the end of the guide, you should be able to create smart contracts and dApps on Arbitrum on xDai. If you want to check details such as RPCs for Arbitrum on xDai, check out our [chainlist page for Arbitrum on xDai](https://thirdweb.com/arbitrum-on-xdai).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Arbitrum on xDai using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Arbitrum on xDai as the network
...
### Initialize SDK On Arbitrum on xDai

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Arbitrum on xDai

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ArbitrumOnXdai } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ArbitrumOnXdai}>
      <YourApp />
    </ThirdwebProvider>
  );
};
