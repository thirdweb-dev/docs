...
# Building contracts and apps on Qitmeer Network Privnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Qitmeer Network Privnet. By the end of the guide, you should be able to create smart contracts and dApps on Qitmeer Network Privnet. If you want to check details such as RPCs for Qitmeer Network Privnet, check out our [chainlist page for Qitmeer Network Privnet](https://thirdweb.com/qitmeer-network-privnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Qitmeer Network Privnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Qitmeer Network Privnet as the network
...
### Initialize SDK On Qitmeer Network Privnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Qitmeer Network Privnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { QitmeerNetworkPrivnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={QitmeerNetworkPrivnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
