...
# Building contracts and apps on Condor Test Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Condor Test Network. By the end of the guide, you should be able to create smart contracts and dApps on Condor Test Network. If you want to check details such as RPCs for Condor Test Network, check out our [chainlist page for Condor Test Network](https://thirdweb.com/condor-test-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Condor Test Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Condor Test Network as the network
...
### Initialize SDK On Condor Test Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Condor Test Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { CondorTestNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={CondorTestNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
