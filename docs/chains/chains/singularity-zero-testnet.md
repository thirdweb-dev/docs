...
# Building contracts and apps on Singularity ZERO Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Singularity ZERO Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Singularity ZERO Testnet. If you want to check details such as RPCs for Singularity ZERO Testnet, check out our [chainlist page for Singularity ZERO Testnet](https://thirdweb.com/singularity-zero-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Singularity ZERO Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Singularity ZERO Testnet as the network
...
### Initialize SDK On Singularity ZERO Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Singularity ZERO Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { SingularityZeroTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={SingularityZeroTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
