...
# Building contracts and apps on DAX CHAIN

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on DAX CHAIN. By the end of the guide, you should be able to create smart contracts and dApps on DAX CHAIN. If you want to check details such as RPCs for DAX CHAIN, check out our [chainlist page for DAX CHAIN](https://thirdweb.com/dax-chain).
...
5. After modifying your contract with your desired custom logic, you may deploy it to DAX CHAIN using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select DAX CHAIN as the network
...
### Initialize SDK On DAX CHAIN

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to DAX CHAIN

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { DaxChain } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={DaxChain}>
      <YourApp />
    </ThirdwebProvider>
  );
};
