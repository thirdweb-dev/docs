...
# Building contracts and apps on BTCIX Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on BTCIX Network. By the end of the guide, you should be able to create smart contracts and dApps on BTCIX Network. If you want to check details such as RPCs for BTCIX Network, check out our [chainlist page for BTCIX Network](https://thirdweb.com/btcix-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to BTCIX Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select BTCIX Network as the network
...
### Initialize SDK On BTCIX Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to BTCIX Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { BtcixNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={BtcixNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
