...
# Building contracts and apps on Meter Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Meter Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Meter Testnet. If you want to check details such as RPCs for Meter Testnet, check out our [chainlist page for Meter Testnet](https://thirdweb.com/meter-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Meter Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Meter Testnet as the network
...
### Initialize SDK On Meter Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Meter Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { MeterTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={MeterTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
