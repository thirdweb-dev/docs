...
# Building contracts and apps on Exosama Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Exosama Network. By the end of the guide, you should be able to create smart contracts and dApps on Exosama Network. If you want to check details such as RPCs for Exosama Network, check out our [chainlist page for Exosama Network](https://thirdweb.com/exosama-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Exosama Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Exosama Network as the network
...
### Initialize SDK On Exosama Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Exosama Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { ExosamaNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={ExosamaNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
