...
# Building contracts and apps on Polis Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Polis Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Polis Testnet. If you want to check details such as RPCs for Polis Testnet, check out our [chainlist page for Polis Testnet](https://thirdweb.com/polis-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Polis Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Polis Testnet as the network
...
### Initialize SDK On Polis Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Polis Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { PolisTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={PolisTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
