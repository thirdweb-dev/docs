...
# Building contracts and apps on OEBlock Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on OEBlock Testnet. By the end of the guide, you should be able to create smart contracts and dApps on OEBlock Testnet. If you want to check details such as RPCs for OEBlock Testnet, check out our [chainlist page for OEBlock Testnet](https://thirdweb.com/oeblock-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to OEBlock Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select OEBlock Testnet as the network
...
### Initialize SDK On OEBlock Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to OEBlock Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { OeblockTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={OeblockTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
