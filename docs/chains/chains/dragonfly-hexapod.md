...
# Building contracts and apps on Dragonfly Mainnet (Hexapod)

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Dragonfly Mainnet (Hexapod). By the end of the guide, you should be able to create smart contracts and dApps on Dragonfly Mainnet (Hexapod). If you want to check details such as RPCs for Dragonfly Mainnet (Hexapod), check out our [chainlist page for Dragonfly Mainnet (Hexapod)](https://thirdweb.com/dragonfly-hexapod).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Dragonfly Mainnet (Hexapod) using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Dragonfly Mainnet (Hexapod) as the network
...
### Initialize SDK On Dragonfly Mainnet (Hexapod)

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Dragonfly Mainnet (Hexapod)

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { DragonflyHexapod } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={DragonflyHexapod}>
      <YourApp />
    </ThirdwebProvider>
  );
};
