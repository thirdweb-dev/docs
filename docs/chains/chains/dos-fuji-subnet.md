...
# Building contracts and apps on Dos Fuji Subnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Dos Fuji Subnet. By the end of the guide, you should be able to create smart contracts and dApps on Dos Fuji Subnet. If you want to check details such as RPCs for Dos Fuji Subnet, check out our [chainlist page for Dos Fuji Subnet](https://thirdweb.com/dos-fuji-subnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Dos Fuji Subnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Dos Fuji Subnet as the network
...
### Initialize SDK On Dos Fuji Subnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Dos Fuji Subnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { DosFujiSubnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={DosFujiSubnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
