...
# Building contracts and apps on XinFin XDC Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on XinFin XDC Network. By the end of the guide, you should be able to create smart contracts and dApps on XinFin XDC Network. If you want to check details such as RPCs for XinFin XDC Network, check out our [chainlist page for XinFin XDC Network](https://thirdweb.com/xinfin-xdc-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to XinFin XDC Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select XinFin XDC Network as the network
...
### Initialize SDK On XinFin XDC Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to XinFin XDC Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { XinfinXdcNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={XinfinXdcNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
