...
# Building contracts and apps on Autobahn Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Autobahn Network. By the end of the guide, you should be able to create smart contracts and dApps on Autobahn Network. If you want to check details such as RPCs for Autobahn Network, check out our [chainlist page for Autobahn Network](https://thirdweb.com/autobahn-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Autobahn Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Autobahn Network as the network
...
### Initialize SDK On Autobahn Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Autobahn Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { AutobahnNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={AutobahnNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
