...
# Building contracts and apps on Fuse Sparknet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Fuse Sparknet. By the end of the guide, you should be able to create smart contracts and dApps on Fuse Sparknet. If you want to check details such as RPCs for Fuse Sparknet, check out our [chainlist page for Fuse Sparknet](https://thirdweb.com/fuse-sparknet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Fuse Sparknet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Fuse Sparknet as the network
...
### Initialize SDK On Fuse Sparknet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Fuse Sparknet
...
const App = () => {
  return (
    <ThirdwebProvider activeChain={FuseSparknet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
