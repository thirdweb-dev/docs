...
# Building contracts and apps on Siberium Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Siberium Network. By the end of the guide, you should be able to create smart contracts and dApps on Siberium Network. If you want to check details such as RPCs for Siberium Network, check out our [chainlist page for Siberium Network](https://thirdweb.com/siberium-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Siberium Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Siberium Network as the network
...
### Initialize SDK On Siberium Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Siberium Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { SiberiumNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={SiberiumNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
