...
# Building contracts and apps on Opulent-X BETA

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Opulent-X BETA. By the end of the guide, you should be able to create smart contracts and dApps on Opulent-X BETA. If you want to check details such as RPCs for Opulent-X BETA, check out our [chainlist page for Opulent-X BETA](https://thirdweb.com/opulent-x-beta).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Opulent-X BETA using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Opulent-X BETA as the network
...
### Initialize SDK On Opulent-X BETA

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Opulent-X BETA

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { OpulentXBeta } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={OpulentXBeta}>
      <YourApp />
    </ThirdwebProvider>
  );
};
