...
# Building contracts and apps on Lucky Network

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Lucky Network. By the end of the guide, you should be able to create smart contracts and dApps on Lucky Network. If you want to check details such as RPCs for Lucky Network, check out our [chainlist page for Lucky Network](https://thirdweb.com/lucky-network).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Lucky Network using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Lucky Network as the network
...
### Initialize SDK On Lucky Network

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Lucky Network

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { LuckyNetwork } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={LuckyNetwork}>
      <YourApp />
    </ThirdwebProvider>
  );
};
