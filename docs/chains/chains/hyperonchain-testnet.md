...
# Building contracts and apps on HyperonChain TestNet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on HyperonChain TestNet. By the end of the guide, you should be able to create smart contracts and dApps on HyperonChain TestNet. If you want to check details such as RPCs for HyperonChain TestNet, check out our [chainlist page for HyperonChain TestNet](https://thirdweb.com/hyperonchain-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to HyperonChain TestNet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select HyperonChain TestNet as the network
...
### Initialize SDK On HyperonChain TestNet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to HyperonChain TestNet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { HyperonchainTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={HyperonchainTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
