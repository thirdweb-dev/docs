...
# Building contracts and apps on Nebula Testnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on Nebula Testnet. By the end of the guide, you should be able to create smart contracts and dApps on Nebula Testnet. If you want to check details such as RPCs for Nebula Testnet, check out our [chainlist page for Nebula Testnet](https://thirdweb.com/nebula-testnet).
...
5. After modifying your contract with your desired custom logic, you may deploy it to Nebula Testnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select Nebula Testnet as the network
...
### Initialize SDK On Nebula Testnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to Nebula Testnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { NebulaTestnet } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={NebulaTestnet}>
      <YourApp />
    </ThirdwebProvider>
  );
};
