...
# Building contracts and apps on eCredits Mainnet

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on eCredits Mainnet. By the end of the guide, you should be able to create smart contracts and dApps on eCredits Mainnet. If you want to check details such as RPCs for eCredits Mainnet, check out our [chainlist page for eCredits Mainnet](https://thirdweb.com/ecredits).
...
5. After modifying your contract with your desired custom logic, you may deploy it to eCredits Mainnet using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select eCredits Mainnet as the network
...
### Initialize SDK On eCredits Mainnet

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to eCredits Mainnet

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { Ecredits } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={Ecredits}>
      <YourApp />
    </ThirdwebProvider>
  );
};
