---
slug: /reference/sdk.split.balanceoftokenallrecipients
title: Split.balanceOfTokenAllRecipients() method
hide_title: true
displayed_sidebar: typescript
---

<!-- Do not edit this file. It is automatically generated by API Documenter. -->

# Split.balanceOfTokenAllRecipients() method

Returns all the recipients and their balances in a non-native currency.

**Signature:**

```typescript
balanceOfTokenAllRecipients(tokenAddress: AddressOrEns): Promise<{
        [key: string]: {
            symbol: string;
            value: BigNumber;
            name: string;
            decimals: number;
            displayValue: string;
        };
    }>;
```

## Parameters

| Parameter    | Type                                  | Description                                           |
| ------------ | ------------------------------------- | ----------------------------------------------------- |
| tokenAddress | [AddressOrEns](./sdk.addressorens.md) | The address of the currency to check the balances in. |

**Returns:**

Promise&lt;{ \[key: string\]: { symbol: string; value: BigNumber; name: string; decimals: number; displayValue: string; }; }&gt;

A map of recipient addresses to their balances in the specified currency.
