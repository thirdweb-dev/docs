---
slug: /reference/sdk.standarderc20.allowanceof
title: StandardErc20.allowanceOf() method
hide_title: true
displayed_sidebar: typescript
---

<!-- Do not edit this file. It is automatically generated by API Documenter. -->

# StandardErc20.allowanceOf() method

Get Token Allowance

## Example

```javascript
// Address of the wallet who owns the funds
const owner = "{{wallet_address}}";
// Address of the wallet to check token allowance
const spender = "0x...";
const allowance = await contract.allowanceOf(owner, spender);
```

**Signature:**

```typescript
allowanceOf(owner: AddressOrEns, spender: AddressOrEns): Promise<CurrencyValue>;
```

## Parameters

| Parameter | Type                                  | Description |
| --------- | ------------------------------------- | ----------- |
| owner     | [AddressOrEns](./sdk.addressorens.md) |             |
| spender   | [AddressOrEns](./sdk.addressorens.md) |             |

**Returns:**

Promise&lt;CurrencyValue&gt;

The allowance of one wallet over anothers funds.

## Remarks

Get the allowance of one wallet over another wallet's funds - the allowance of a different address for a token is the amount of tokens that the wallet is allowed to spend on behalf of the specified wallet.
