---
slug: /reference/sdk.userwallet.transfer
title: UserWallet.transfer() method
hide_title: true
displayed_sidebar: typescript
---

<!-- Do not edit this file. It is automatically generated by API Documenter. -->

# UserWallet.transfer() method

Transfer native or ERC20 tokens from this wallet to another wallet

## Example

```javascript
// transfer 0.8 ETH
await sdk.wallet.transfer("0x...", 0.8);
// transfer 0.8 tokens of `tokenContractAddress`
await sdk.wallet.transfer("0x...", 0.8, tokenContractAddress);
```

**Signature:**

```typescript
transfer(to: AddressOrEns, amount: Amount, currencyAddress?: AddressOrEns): Promise<TransactionResult>;
```

## Parameters

| Parameter       | Type                                  | Description                                                             |
| --------------- | ------------------------------------- | ----------------------------------------------------------------------- |
| to              | [AddressOrEns](./sdk.addressorens.md) | the account to send funds to                                            |
| amount          | [Amount](./sdk.amount.md)             | the amount in tokens                                                    |
| currencyAddress | [AddressOrEns](./sdk.addressorens.md) | _(Optional)_ Optional - ERC20 contract address of the token to transfer |

**Returns:**

Promise&lt;[TransactionResult](./sdk.transactionresult.md)&gt;
