...
<h3 id="active-chain"> activeChain (recommended) </h3>
...
<h3 id="custom-chains"> Custom EVM Chains </h3>
...
<h3 id="supportedWallets"> supportedWallets (optional) </h3>

An array of Wallets that your app supports.

If not provided, defaults to `MetaMaskWallet`, `CoinbaseWallet` and `DeviceWallet`.

( other supported wallets are `WalletConnectv1`, `WalletConnect` ( v2 ) )

```jsx title="Configuring Metamask, Coinbase and Wallet Connect (v2) "
import {
  ThirdwebProvider,
  MetamaskWallet,
  CoinbaseWallet,
...
function MyApp() {
  return (
    <ThirdwebProvider
      walletConnectors={[MetamaskWallet, CoinbaseWallet, WalletConnect]}
      activeChain="mumbai"
    >
      <App />
    </ThirdwebProvider>
...
This Metadata is passed to the wallet. Some wallets you this information to display the metadata of the app to the user when a user is connecting the wallet to your app.
...
`authConfig` is not yet functional in the Connect Wallet v2 `@nightly` release
:::
