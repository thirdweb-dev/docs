...
If not provided, defaults to `metamaskWallet`, `coinbaseWallet` and `WalletConnectV1`.

( other supported wallets are `paperWallet`, `safeWallet`, `walletConnect` ( v2 ) )
...
function MyApp() {
  return (
    <ThirdwebProvider
      walletConnectors={[metamaskWallet(), coinbaseWallet(), WalletConnectV1()]}
      activeChain="mumbai"
    >
      <App />
    </ThirdwebProvider>
