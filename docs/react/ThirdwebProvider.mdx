...
The `clientId` prop is required to use the thirdweb infrastructure services with the SDK. You can get a client ID by [creating an API key](/dashboard/settings) [here](https://thirdweb.com/create-api-key).
