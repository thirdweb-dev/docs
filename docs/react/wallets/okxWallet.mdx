...
A wallet configurator for [OKX Wallet](/wallet/okx-wallet) which allows integrating the wallet with React.

```tsx
import { okxWallet } from "@thirdweb-dev/react";

const okxWalletConfig = okxWallet(options);
```
...
```tsx
<ThirdwebProvider supportedWallets={[okxWallet()]} clientId="your-client-id">
  <YourApp />
</ThirdwebProvider>
```
...
function App() {
  ...
  const handleConnect = async () => {
    await connect(okxWalletConfig, connectOptions);
  };
