...
export default function Component() {
  ...
  const {
    mutateAsync: updateCreators,
    isLoading,
    error,
  } = useUpdateCreators(program);

  return (
    <button onClick={() => updateCreators([{ address: "0x...", share: 10 }])}>
      Update Creators
    </button>
  );
}
...
| --------- | ------------------------------------------------- | --------------------------------------------------- |
| program   | RequiredParam&lt;NFTCollection &#124; NFTDrop&gt; | The NFT program instance to update the creators for |
