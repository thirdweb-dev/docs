...
Calling the function returned by the `usePaperWallet` hook opens the Paper Wallet's Modal and prompts the user to login with their email address.

If `email` is not provided, user will be prompted to enter their email or sign-in with Google account. Once the user enters email or signs-in with Google account, an OTP will be sent to the user's email address. Once User enters the OTP, wallet will be connected.

If email is provided, user will be directly shown the OTP screen - user will not be prompted to enter their email.
...
If user's email is already known, you can skip prompting the user for entering email address and directly show the OTP screen by passing the `email` in a configuration object as shown below.
