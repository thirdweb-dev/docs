...
There are two ways to call this `conect` function: using email or phone number.

If you call the `connect` function with `email`, a modal will open and prompt the user to check their mail and click on the link sent to their email. Once user completes this step, the modal will close and the user will be connected to your app.
    ...
    <>
      ...
      <button
        onClick={() => {
          connectWithMagic({
            apiKey: "YOUR_API_KEY",
            email: "+123456789", // user's phone number
          });
        }}
      >
