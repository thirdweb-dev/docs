...
slug: /react/react.useenglishauctionwinningbid
...
function App() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useEnglishAuctionWinningBid(contract, auctionId);
}
```

## Return Value

The hook's `data` property, once loaded, will be an object of type `Bid`, containing the following properties:
