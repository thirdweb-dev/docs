...
1. There is a **connected wallet**
   (if there is not, renders a [Connect Wallet Button](/react/react.connectwallet) instead).
...
The contract instance is passed as the first argument to the function
