...
1. There is a **connected wallet**
   (if there is not, it renders a [Connect Wallet Button](/react/react.connectwallet) instead).
...
:::info
If the action you are performing is async, Make sure to return a `Promise` from the `action` function so that SDK knows when the action is complete
