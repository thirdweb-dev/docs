...
import ThirdwebNftMediaEditor from "@components/live-code/snippets/ThirdwebNftMediaEditor";

Component that renders an NFT from given a `metadata` object.

Under the hood, if the `image` property of the metadata is a URL/IPFS URI, it is fetched from the source.
...
The `image` is used as the media, and the `name` is used as the alt text for the media.
