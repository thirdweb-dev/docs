---
slug: /react.usebuyoutlisting
title: useBuyoutListing() function
hide_title: true
...
```typescript
export declare function useBuyoutListing(
...
const Component = () => {
  ...
  return (
    <button disabled={isLoading} onClick={() => buyoutListing(listingId)}>
      Buy listing!
    </button>
  );
};
