...
Below packages are required to be installed as dev dependencies to use thirdweb SDK with Create React App.
...
export const ThirdwebProviderClient: typeof ThirdwebProvider = (props) => (
  <ThirdwebProvider {...props} />
);

// app/components/ThirdwebProviderClient.tsx
```
...
export default function Home() {
  return <ThirdwebProviderClient> ... </ThirdwebProviderClient>;
}
...
:::caution API KEYS
You will require an API key to use thirdweb's infrastructure services with the SDK. If you haven'tt created a key yet you can do so for **free** from the [**thirdweb dashboard**](https://thirdweb.com/create-api-key).
:::
