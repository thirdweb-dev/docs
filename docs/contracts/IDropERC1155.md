...
| _tokenId | uint256 | The unique ID of the token to claim.
...
```solidity
function setClaimConditions(uint256 _tokenId, IDropClaimCondition.ClaimCondition[] _conditions, bool _resetLimitRestriction) external nonpayable
```

Lets a module admin (account with `DEFAULT_ADMIN_ROLE`) set mint conditions for a given token ID.
...
| Name | Type | Description |
|---|---|---|
| _tokenId | uint256 | The token ID for which to set mint conditions.
| _conditions | IDropClaimCondition.ClaimCondition[] | Mint conditions in ascending order by `startTimestamp`.
| _resetLimitRestriction | bool | Whether to reset the claim limit restriction.
