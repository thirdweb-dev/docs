...
# Building contracts and apps on {chainName}

This guide will look at how you can create and deploy smart contracts and create apps to interact with these contracts deployed on {chainName}. By the end of the guide, you should be able to create smart contracts and dApps on {chainName}. If you want to check details such as RPCs for {chainName}, check out our [chainlist page for {chainName}](https://thirdweb.com/{chainSlug}).
...
5. After modifying your contract with your desired custom logic, you may deploy it to {chainName} using [Deploy](https://portal.thirdweb.com/deploy).
...
3. Select {chainName} as the network
...
### Initialize SDK On {chainName}

Wrap your application in the `ThirdwebProvider` component and change the `activeChain` to {chainName}

```jsx
import { ThirdwebProvider } from "@thirdweb-dev/react";
import { {chainExport} } from "@thirdweb-dev/chains";

const App = () => {
  return (
    <ThirdwebProvider activeChain={{ chainExport }}>
      <YourApp />
    </ThirdwebProvider>
  );
};
