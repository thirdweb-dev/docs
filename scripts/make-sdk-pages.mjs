...
(async () => {
  ...
  const generateContractLink = (contractName) => {
    ...
    // /contract-kit/erc-721/${contractName}
    // If it contains ERC1155, then the link is :
    // /contract-kit/erc-1155/${contractName}
    // If it contains ERC20, then the link is :
    // /contract-kit/erc-20/${contractName}
    // Otherwise the link is :
    // /contract-kit/features/${contractName}
    let output = "";

    if (contractName.includes("ERC721")) {
      output = `/contract-kit/erc-721/${contractName}`;
    } else if (contractName.includes("ERC1155")) {
      output = `/contract-kit/erc-1155/${contractName}`;
    } else if (contractName.includes("ERC20")) {
      output = `/contract-kit/erc-20/${contractName}`;
    } else {
      output = `/contract-kit/features/${contractName}`;
    }
