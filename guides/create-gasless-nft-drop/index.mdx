...
I am going to use an ERC-1155 so multiple people can hold the same NFT but you can do it any way you like! [Check out our NFT contracts](https://portal.thirdweb.com/pre-built-contracts/nfts).
...
To add wallet connection, we are going to use the `useMetamask` hook that thirdweb provides, so add the hook like the following:
