...
In this guide, we will allow users to claim ERC-20 tokens in our nextJS app and show holders of our tokens!
<!-- truncate -->

## Intro

Just like our [NFT Drop](https://portal.thirdweb.com/pre-built-contracts/nft-drop) contract, the [Token Drop](https://portal.thirdweb.com/pre-built-contracts/token-drop) contract makes the token available for users to claim. In other words, the actual minting happens when a user claims the token. In this app a user can claim our token. Right at that time the ERC-20 token is minted and sent to the wallet of that user. This app also shows all the wallets that hold this token. 
...
First deploy a `Token Drop` contract from [thirdweb’s dashboard](https://thirdweb.com/dashboard) by clicking on `+ Deploy new contract`. Navigate to the `Token Drop` contract by clicking on `Pre-built contracts` and then `Release a drop`. Don't forget to set your claim conditions under `Claim Phases`.  
Next go ahead and run the following command to get the NextJS Javascript starter template. 
```bash
npx thirdweb create --next --js
```
Inside the `_app.js` file, change the chain ID to the chain you deployed the smartcontract too.
```jsx
...
We’re going to use the `claim` method in our sdk. The `claim` method takes one parameter, which is the amount of tokens to be claimed. In this example we’ll let the user pass the amount, but you can pre-define this.

```jsx
function claim () {
	const claimResult = tokenDropContract?.claim(<AMOUNT>);
}
```

Because we we’re allowing the user to enter the amount, we need to make use of the react hook `useState` to allow our app to handle the dynamic input.
...
//updated method
function claim () {
	const claimResult = tokenDropContract?.claim(amountToClaim);
}
```
...
```jsx
import {useTokenDrop, useAddress} from "@thirdweb-dev/react";
...
Now to render out a list of our token holders, we’ll use the `getAllHolderBalances` method. It’s straight forward, build the function and render it out inside the component like this.
...
Now we need to render out the data inside our page. `balances` fetches data in a JSON format. We can use a `map` method to render out all the holders and their balances like this.

```jsx
.map((holder) => (
            <div key={holder.holder}>
              <p>{holder.holder}</p>
              <p>{holder.balance.displayValue} {holder.balance.symbol}</p>
            </div>
```

Here is the full page, including a loading view and sorting the balances 👇
...
That’s how you integrate the `Token Drop` contract with a `NextJS` app. This guide is just an example of how to do it. Again we went over the core logic of the app. Feel free to go to the [repo](https://github.com/thirdweb-example/token-drop) to get a full copy and start building!