...
Inside our `src` folder, we will open the `index.js` file.
We will first change the chain id to the chain of our NFT Drop.
For this project, we’ll be using `Mumbai` since our NFT Drop contract is on the Mumbai testnet.
...
We have one button that lets users connect with MetaMask.
