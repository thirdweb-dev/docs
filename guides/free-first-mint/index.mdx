...
<div className="col col--6" style={{ marginTop: 8 }}>
  <QuickstartCard
    ...
    link="https://github.com/thirdweb-dev/examples/tree/main/typescript/free-first-mint"
...
After you create a new base, give a name to your base and add two columns: `address` and `quantityClaimed`. Address will be of type `Single Line Text` and quantityClaimed will be of type `Number`:
...
![Airtable App ID](./airtable-app-id.png)

Create a new `.env.local` at the root of your project and add three new variables:

The table name is the name of the table you created in Airtable.
...
```ts title="pages/api/generate-mint-sig.ts"
...
Let's now initialize the [thirdweb SDK](/building-web3-apps/setting-up-the-sdk)! In a new file called `thirdweb.ts` inside the `utils` folder:
...
```ts title="pages/api/generate-mint-sig.ts"
// Replace this address below with your signature drop contract address
// (You can get this from the dashboard)
...
We will import SDK from the utils folder as well as our Airtable:
...
To fix this after generating the signature we will update the `quantityClaimed` in the airtable. So, add the following:
...
Here, we are first getting the record using the address. If the record exists, we will just update it by adding the `quantityToMint` to previous `quantityClaimed` and if it doesn't exist, we will create a new record with the address and set it to `quantityToMint`.

The final code for our API looks like this:
...
We are updating the airtable while generating the signature so what if we get an error or the user rejects the signature? So, we will create a new route called `fix-airtable`. Create a new file called `pages/api/fix-airtable.ts` and add the following:

```tsx title="api/fix-airtable.ts"
import type { NextApiRequest, NextApiResponse } from "next";
import { table } from "../../utils/Airtable";
...
Here we are receiving the address and quantity from the user and then we are subtracting the quantity from the quantity claimed in the airtable. We are also making sure that no one can make api requests from outside the website and tamper the data by checking for same origin!

If we test out the flow now, everything works perfectly 🥳!
