...
```ts title='pages/api/set-claimed.ts'
import type { NextApiRequest, NextApiResponse } from "next";
import table from "../../utils/Airtable";
import sdk from "../../utils/thirdweb";
...
Here, we are first getting the record using the address. If the record exists, we will just update it with `hasClaimed` to `true`,
otherwise, we will create a new record.

We are also making sure that the address owns an NFT so someone can't make a false API call and mark an address as
claimed even though it hasn't claimed any NFT yet.
