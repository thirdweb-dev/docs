...
# Deploy an NFT Drop with revenue share without writing any code

Learn how to setup a revenue share on your NFT drop with no-code.
...
For this contract, we're going to be deploying on **Ethereum Goerli Testnet**, but you can use a mainnet for it (Ethereum, Polygon, Avalanche, etc).

To create your split contract, click on "Deploy new contract", then "Deploy Now" on the **Split** pre-built contract.

Here is the **Split** contract information we need to provide:

1. **Name**: this is the name of your split contract.
2. **Image** (optional): add a cover image for your split contract here.
3. **Description** (optional): add a description for your split contract here.
4. **Wallet Recipients**: these are the wallets included in the split. Add the wallets for your split, along with the % allocation of each wallet. All of these wallets will be able to see their portion of the funds in the contract on the dashboard. All wallets can also distribute the funds.
5. **Percentage**: the percent allocation for each wallet of the total revenue received.

![Input your Split contract information](./deploy-new-split-contract.png)
...
![Deployed Split contract on dashboard](./deployed-split-contract.png)
...
![Split contract address](./split-contract-address.png)

### Deploy to your drop

Next up, we will deploy our NFT drop and connect our split contract.

For this contract, we're going to deploy on the same network - **Goerli**.

To create your NFT Drop contract, click on "Deploy new contract", then hit "Deploy Now", on the **NFT Drop** contract.
...
![NFT drop with Split contract address as sales recipient](./deploy-nft-drop.png)
...
For this guide, we’ll assume you know how to set up the rest of your NFT Drop. For more information on how to complete your NFT drop, check out [this](https://blog.thirdweb.com/guides/release-an-nft-drop-with-no-code) guide.

Next, will show you your split contract after you receive the revenue.
...
This step shows you how to distribute ALL the funds in the split contract, and correctly allocate them to each wallet listed.
...
![Split contract with your wallet balance](./split-contract-with-funds.png)
...
![Select 'Distribute Funds'](./select-distribute-funds.png)

Lastly, we can open our transaction on Goerli Etherscan to make sure our funds were distributed properly:

![Etherscan confirmation](./etherscan-confirmation.png)
