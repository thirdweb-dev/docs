...
and create a project and a **Edition** contract.
...
I'll be using React for the front of our app, you can use anything you're comfortable with.
...
```bash
pip install fastapi uvicorn thirdweb-sdk firebase-admin textwrap Pillow python-dotenv
```

Let's first write the code for minting a NFT, then we'll wrap it up in a simple API.
...
That way, you know when the image and subsequently the NFT was created.

It then puts the edited image on Firebase and gets back a URL to access the image.

You'll have to setup a Firebase project and get your JSON keys + bucket name from there for this to work. Checkout these [docs](https://firebase.google.com/docs/reference/admin/python) for more information.
...
def mint(item: Item):
  ...
  Datestamp - """ + dt_string
    ...
    sdk = ThirdwebSdk(SdkOptions(), "polygon")
