...
Learn how to create a dynamic allowlist where specific wallets can mint your ERC-1155 NFT and we will also see how to allow them to limit them to only 1 NFT!

<!-- truncate -->

## Introduction

In this guide, we are going to create an [Edition](/pre-built-contracts/edition) where people will be able to sign up their wallets to an allowlist and those wallets can later mint the NFT!
...
const [nftData, setNftData] =
  (useState < EditionMetadata["metadata"]) | (null > null);
```
...
You will now get an error because we are using the next `<Image />` component but haven't allowlisted the domain, so go to `next.config.js` and add the following:
...
![Create Alchemy app](./create-alchemy-app.png)
